package crypto.forestfish.objects.embedded.evm;

import java.util.ArrayList;
import java.util.HashMap;

import crypto.forestfish.enums.BlockchainType;
import crypto.forestfish.enums.evm.EVMChain;
import crypto.forestfish.enums.evm.EVMPriceMechanism;
import crypto.forestfish.objects.evm.model.chain.EVMChainInfo;
import crypto.forestfish.objects.evm.model.chain.EVMCurrency;

public class BlockchainDetailsMainEVM2 {

	@SuppressWarnings("serial")
	public static HashMap<EVMChain, EVMChainInfo> getMainnetEVMNetworks() {

		HashMap<EVMChain, EVMChainInfo> networks = new HashMap<>();

		// onnchain
		EVMChainInfo onnchain = new EVMChainInfo(
				EVMChain.ONNCHAIN.toString(), 
				"ONNChain", 
				1330L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("ONNChain Gas Token", "ONN", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet.onnscan.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://onnscan.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1330");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ONNCHAIN, onnchain);
		
		// inri
		EVMChainInfo inri = new EVMChainInfo(
				EVMChain.INRI.toString(), 
				"INRI CHAIN", 
				3777L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("INRI CHAIN Gas Token", "INRI", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.inri.life");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.inri.life");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/3777");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.INRI, inri);
		
		// tempo
		EVMChainInfo tempo = new EVMChainInfo(
				EVMChain.TEMPO.toString(), 
				"Tempo Mainnet", 
				4217L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Tempo Mainnet Gas Token", "USD", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://tempo-mainnet.drpc.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explore.mainnet.tempo.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/4217");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.TEMPO, tempo);
		
		// creator
		EVMChainInfo creator = new EVMChainInfo(
				EVMChain.CREATOR.toString(), 
				"Creator Chain Mainnet", 
				2787L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Creator Chain Mainnet Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.mainnet.oncreator.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.mainnet.oncreator.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/2787");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.CREATOR, creator);
		
		// datachain
		EVMChainInfo datachain = new EVMChainInfo(
				EVMChain.DATACHAINROPE.toString(), 
				"Datachain Rope", 
				271828L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Datachain Rope Gas Token", "FAT", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://erpc.datachain.network");
					this.add("https://ws.datachain.network");
					this.add("https://erpc.rope.network");
				}},
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://dcscan.io");
				}},
				new ArrayList<String>() {{
					this.add("https://faucet.datachain.network");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/271828");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.DATACHAINROPE, datachain);
		
		// fanatico
		EVMChainInfo fanatico = new EVMChainInfo(
				EVMChain.FANATICO.toString(), 
				"Fanatico", 
				11111111111L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Fanatico Gas Token", "FCO", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.fanati.co");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.fanati.co");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/11111111111");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.FANATICO, fanatico);
		
		// blockdag
		EVMChainInfo blockdag = new EVMChainInfo(
				EVMChain.BLOCKDAG.toString(), 
				"BlockDAG Network", 
				1404L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("BlockDAG Network Gas Token", "BDAG", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.bdagscan.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://bdagscan.com/");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1404");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.BLOCKDAG, blockdag);

		// ebchain
		EVMChainInfo ebchain = new EVMChainInfo(
				EVMChain.EBCHAIN.toString(), 
				"EB-Chain", 
				8721L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("EB-Chain Gas Token", "EBC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.ebcscan.net");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://ebcscan.net");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/8721");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.EBCHAIN, ebchain);
		
		// irys
		EVMChainInfo irys = new EVMChainInfo(
				EVMChain.IRYS.toString(), 
				"Irys Mainnet Beta", 
				3282L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Irys Mainnet Beta Gas Token", "IRYS", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet-beta-rpc.irys.xyz/v1/execution-rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://evm-explorer.irys.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/3282");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.IRYS, irys);
		
		// bytechain
		EVMChainInfo bytechain = new EVMChainInfo(
				EVMChain.BYTECHAIN.toString(), 
				"ByteChain Mainnet", 
				9933L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("ByteChain Mainnet Gas Token", "BEXC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.bexc.io");
					this.add("https://rpc2.bexc.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://mainnet.bexc.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/9933");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.BYTECHAIN, bytechain);
		
		// qelt
		EVMChainInfo qelt = new EVMChainInfo(
				EVMChain.QELT.toString(), 
				"QELT Mainnet", 
				770L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("QELT Mainnet Gas Token", "QELT", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet.qelt.ai");
					this.add("https://mainnet2.qelt.ai");
					this.add("https://mainnet3.qelt.ai");
					this.add("https://mainnet4.qelt.ai");
					this.add("https://mainnet5.qelt.ai");
					this.add("https://archivem.qelt.ai");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://qeltscan.ai");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/770");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.QELT, qelt);
		
		// megaeth
		EVMChainInfo megaeth = new EVMChainInfo(
				EVMChain.MEGAETH.toString(), 
				"MegaETH", 
				4326L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("MegaETH Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet.megaeth.com/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://mega.etherscan.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/4326");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.MEGAETH, megaeth);
		
		// opn
		EVMChainInfo opn = new EVMChainInfo(
				EVMChain.OPN.toString(), 
				"OPN Chain Mainnet", 
				222L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("OPN Chain Mainnet Gas Token", "OPN", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet-rpc.iopn.tech");
					this.add("https://mainnet-rpc2.iopn.tech");
					this.add("https://mainnet-rpc3.iopn.tech");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://opnscan.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/222");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.OPN, opn);
		
		// xgr
		EVMChainInfo xgr = new EVMChainInfo(
				EVMChain.XGR.toString(), 
				"XGR Mainnet", 
				1643L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("XGR Mainnet Gas Token", "XGR", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.xgr.network");
					this.add("https://rpc1.xgr.network");
					this.add("https://rpc2.xgr.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.xgr.network");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1643");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.XGR, xgr);
		
		// dolphinet
		EVMChainInfo dolphinet = new EVMChainInfo(
				EVMChain.DOLPHINET.toString(), 
				"Dolphinet Mainnet", 
				1520L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Dolphinet Mainnet Gas Token", "DOL", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.dolphinode.world");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.dolphinode.world/");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1520");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.DOLPHINET, dolphinet);
		
		// beechain
		EVMChainInfo beechain = new EVMChainInfo(
				EVMChain.BEECHAIN.toString(), 
				"BeeChain Mainnet", 
				3188L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("BeeChain Mainnet Gas Token", "BKC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.beechain.ai");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.beechain.ai");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/3188");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.BEECHAIN, beechain);

		// jasmychain
		EVMChainInfo jasmychain = new EVMChainInfo(
				EVMChain.JASMYCHAIN.toString(), 
				"JasmyChain", 
				680L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("JasmyChain Gas Token", "JASMY", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.jasmyscan.net");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.jasmyscan.net");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/680");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.JASMYCHAIN, jasmychain);
		
		// kiteai
		EVMChainInfo kiteai = new EVMChainInfo(
				EVMChain.KITEAI.toString(), 
				"KiteAI", 
				2366L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("KiteAI Gas Token", "KITE", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.gokite.ai");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://kitescan.ai");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/2366");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.KITEAI, kiteai);
		
		// krown
		EVMChainInfo krown = new EVMChainInfo(
				EVMChain.KROWN.toString(), 
				"Krown Mainnet", 
				1983L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Krown Mainnet Gas Token", "KROWN", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet.krown.network");
					this.add("https://mainnet1.krown.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.krown.network");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1983");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.KROWN, krown);
		
		// luxeports
		EVMChainInfo luxeports = new EVMChainInfo(
				EVMChain.LUXEPORTS.toString(), 
				"LuxePorts", 
				1122L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("LuxePorts Gas Token", "LXP", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.luxeports.com");
					this.add("https://erpc.luxeports.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://lxpscan.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1122");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.LUXEPORTS, luxeports);

		
		// validium
		EVMChainInfo validium = new EVMChainInfo(
				EVMChain.VALIDIUM.toString(), 
				"Validium Network", 
				567L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Validium Network Gas Token", "VLDM", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://testnet.l2.rpc.validium.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://testnet.explorer.validium.network");
				}},
				new ArrayList<String>() {{
					this.add("https://testnet.faucet.validium.network");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/567");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.VALIDIUM, validium);

		// valuechain
		EVMChainInfo valuechain = new EVMChainInfo(
				EVMChain.VALUECHAIN.toString(), 
				"ValueChain", 
				286623L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("ValueChain Gas Token", "SOSO", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet.valuechain.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://main-scan.valuechain.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/286623");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.VALUECHAIN, valuechain);
		
		// pharos
		EVMChainInfo pharos = new EVMChainInfo(
				EVMChain.PHAROS.toString(), 
				"Pharos Mainnet", 
				1672L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Pharos Mainnet Gas Token", "PROS", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.pharos.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://pharosscan.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1672");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.PHAROS, pharos);
		
		// bggchain
		EVMChainInfo bggchain = new EVMChainInfo(
				EVMChain.BGGCHAIN.toString(), 
				"BGGChain", 
				3005L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("BGGChain Gas Token", "BGG", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.bggchain.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://bggscan.com/");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/3005");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.BGGCHAIN, bggchain);
		
		// aetron
		EVMChainInfo aetron = new EVMChainInfo(
				EVMChain.AETRON.toString(), 
				"AETRON Mainnet", 
				3355L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("AETRON Mainnet Gas Token", "AET", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://entrypoint-mainnet.aetron.ai/");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/3355");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.AETRON, aetron);
		
		// ethiq
		EVMChainInfo ethiq = new EVMChainInfo(
				EVMChain.ETHIQ.toString(), 
				"Ethiq", 
				30303L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Ethiq Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.ethiq.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.ethiq.haqq.network");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/30303");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ETHIQ, ethiq);
		
		// l2
		EVMChainInfo l2 = new EVMChainInfo(
				EVMChain.L2.toString(), 
				"L2 Protocol Mainnet", 
				12216L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("L2 Protocol Mainnet Gas Token", "L2P", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.l2protocol.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://l2pscan.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/12216");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.L2, l2);
		
		// reactive
		EVMChainInfo reactivel = new EVMChainInfo(
				EVMChain.REACTIVELASNA.toString(), 
				"Reactive Lasna", 
				5318007L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Reactive Lasna Gas Token", "lREACT", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://lasna-rpc.rnk.dev");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://lasna.reactscan.net");
				}},
				new ArrayList<String>() {{
					this.add("https://dev.reactive.network/reactive-mainnet#get-testnet-react");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/5318007");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.REACTIVELASNA, reactivel);
		
		// monad
		EVMChainInfo monad = new EVMChainInfo(
				EVMChain.MONAD.toString(), 
				"Monad", 
				143L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Monad Gas Token", "MON", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.monad.xyz");
					this.add("https://rpc1.monad.xyz");
					this.add("https://rpc3.monad.xyz");
					this.add("https://rpc-mainnet.monadinfra.com");
					this.add("https://rpc2.monad.xyz");
					this.add("https://rpc4.monad.xyz");
					this.add("https://monad.rpc.blxrbdn.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://monadscan.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/143");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.MONAD, monad);
		
		// obsidian
		EVMChainInfo obsidian = new EVMChainInfo(
				EVMChain.OBSIDIAN.toString(), 
				"Obsidian Mainnet", 
				1719L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Obsidian Mainnet Gas Token", "OBS", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://kr-4.pixelzx.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1719");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.OBSIDIAN, obsidian);
		
		// globalchain
		EVMChainInfo globalchain = new EVMChainInfo(
				EVMChain.GLOBALCHAIN.toString(), 
				"GLOBALCHAIN", 
				38391207L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("GLOBALCHAIN Gas Token", "GBDo", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.brantley-global.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://brantley-global.com/dashboard");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/38391207");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.GLOBALCHAIN, globalchain);
		
		// parallax
		EVMChainInfo parallax = new EVMChainInfo(
				EVMChain.PARALLAX.toString(), 
				"Parallax", 
				2110L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Parallax Gas Token", "LAX", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.parallaxchain.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.parallaxchain.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/2110");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.PARALLAX, parallax);
		
		// dmd
		EVMChainInfo dmd = new EVMChainInfo(
				EVMChain.DMD.toString(), 
				"DMD Diamond", 
				17771L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("DMD Diamond Gas Token", "DMD", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.bit.diamonds");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.bit.diamonds");
				}},
				new ArrayList<String>() {{
					this.add("https://faucet.bit.diamonds");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/17771");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.DMD, dmd);
		
		// henesys
		EVMChainInfo henesys = new EVMChainInfo(
				EVMChain.HENESYS.toString(), 
				"Henesys", 
				68414L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Henesys Gas Token", "NXPC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://henesys-rpc.msu.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://subnets.avax.network/henesys");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/68414");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.HENESYS, henesys);
		
		// cx
		EVMChainInfo cx = new EVMChainInfo(
				EVMChain.CX.toString(), 
				"CX Chain Mainnet", 
				737373L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("CX Chain Mainnet Gas Token", "CX", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://subnets.avax.network/cx/mainnet/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://subnets.avax.network/cx");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/737373");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.CX, cx);
		
		// amazonic
		EVMChainInfo amazonic = new EVMChainInfo(
				EVMChain.AMAZONIC.toString(), 
				"Amazonic Blockchain", 
				65L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Amazonic Blockchain Gas Token", "AMZG", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.amz1.com.br");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://amzexplorer.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/65");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.AMAZONIC, amazonic);
		
		// lorachain
		EVMChainInfo lorachain = new EVMChainInfo(
				EVMChain.LORACHAIN.toString(), 
				"LoraChain Mainnet", 
				6359L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("LoraChain Mainnet Gas Token", "LRB", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.lorachain.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.lorachain.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/6359");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.LORACHAIN, lorachain);
		
		// mevcommit
		EVMChainInfo mevcommit = new EVMChainInfo(
				EVMChain.MEVCOMMIT.toString(), 
				"mev-commit Mainnet", 
				57173L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("mev-commit Mainnet Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://chainrpc.mev-commit.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://www.mev-commit.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/57173");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.MEVCOMMIT, mevcommit);
		
		// lazai
		EVMChainInfo lazai = new EVMChainInfo(
				EVMChain.LAZAI.toString(), 
				"LazAI Mainnet", 
				52924L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("LazAI Mainnet Gas Token", "METIS", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet.lazai.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.mainnet.lazai.network");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/52924");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.LAZAI, lazai);
		
		// denergy
		EVMChainInfo denergy = new EVMChainInfo(
				EVMChain.DENERGY.toString(), 
				"Denergy Network", 
				369369L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Denergy Network Gas Token", "WATT", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.d.energy");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.d.energy");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/369369");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.DENERGY, denergy);
		
		// sintrop
		EVMChainInfo sintrop = new EVMChainInfo(
				EVMChain.SINTROP.toString(), 
				"Sintrop Impact Blockchain", 
				250225L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Sintrop Impact Blockchain Gas Token", "SIN", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.sintrop.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.sintrop.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/250225");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.SINTROP, sintrop);
		
		// kultchain
		EVMChainInfo kultchain = new EVMChainInfo(
				EVMChain.KULTCHAIN.toString(), 
				"KultChain", 
				220312L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("KultChain Gas Token", "KLT", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.kultchain.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.kultchain.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/220312");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.KULTCHAIN, kultchain);
		
		// atrnxos
		EVMChainInfo atrnxos = new EVMChainInfo(
				EVMChain.ATRNXOS.toString(), 
				"ATRNXOS Chain", 
				19966L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("ATRNXOS Chain Gas Token", "EATB", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.bas.atrnx.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.bas.atrnx.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/19966");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ATRNXOS, atrnxos);
		
		// turkchain
		EVMChainInfo turkchain = new EVMChainInfo(
				EVMChain.TURKCHAIN.toString(), 
				"TURKCHAIN", 
				1919L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("TURKCHAIN Gas Token", "TC-20", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.turkchain1919.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.turkchain1919.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1919");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.TURKCHAIN, turkchain);
		
		// recordercoin
		EVMChainInfo recordercoin = new EVMChainInfo(
				EVMChain.RECORDERCOIN.toString(), 
				"RecorderCoin Mainnet", 
				1888L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("RecorderCoin Mainnet Gas Token", "RECR", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.recordercoin.org");
					this.add("https://rpc1.recordercoin.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.recordercoin.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1888");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.RECORDERCOIN, recordercoin);
		
		// panchain
		EVMChainInfo panchain = new EVMChainInfo(
				EVMChain.PANCHAIN.toString(), 
				"Panchain Mainnet", 
				7890L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Panchain Mainnet Gas Token", "PC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://publicrpc.panchain.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.panchain.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/7890");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.PANCHAIN, panchain);
		
		// potos
		EVMChainInfo potos = new EVMChainInfo(
				EVMChain.POTOS.toString(), 
				"POTOS Mainnet", 
				60603L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("POTOS Mainnet Gas Token", "POT", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.potos.hk");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.potos.hk");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/60603");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.POTOS, potos);
		
		// warden
		EVMChainInfo warden = new EVMChainInfo(
				EVMChain.WARDEN.toString(), 
				"Warden", 
				8765L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Warden Gas Token", "WARD", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://evm.wardenprotocol.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.wardenprotocol.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/8765");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.WARDEN, warden);
		
		// autonomys
		EVMChainInfo autonomys = new EVMChainInfo(
				EVMChain.AUTONOMYS.toString(), 
				"Autonomys Mainnet", 
				870L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Autonomys Mainnet Gas Token", "AI3", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://auto-evm.mainnet.autonomys.xyz/ws");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/870");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.AUTONOMYS, autonomys);
		
		// satoshivm
		EVMChainInfo satoshivm = new EVMChainInfo(
				EVMChain.SATOSHIVM.toString(), 
				"SatoshiVM", 
				3109L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("SatoshiVM Gas Token", "BTC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://alpha-rpc-node-http.svmscan.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://svmscan.io/");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/3109");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.SATOSHIVM, satoshivm);
		
		// galaxy
		EVMChainInfo galaxy = new EVMChainInfo(
				EVMChain.GALAXY.toString(), 
				"Galaxy Chain", 
				420420420420L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Galaxy Chain Gas Token", "STAR", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://archive.galaxychain.co");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.galaxychain.co");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/420420420420");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.GALAXY, galaxy);
		
		// weichain
		EVMChainInfo weichain = new EVMChainInfo(
				EVMChain.WEICHAIN.toString(), 
				"Weichain net", 
				37771L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Weichain net Gas Token", "WeiC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("http://1.15.137.12:8545");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("http://1.15.137.12:5200/");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/37771");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.WEICHAIN, weichain);
		
		// ab
		EVMChainInfo ab = new EVMChainInfo(
				EVMChain.AB.toString(), 
				"AB Core Mainnet", 
				36888L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("AB Core Mainnet Gas Token", "AB", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.core.ab.org");
					this.add("https://rpc1.core.ab.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.core.ab.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/36888");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.AB, ab);
		
		// slcchain
		EVMChainInfo slcchain = new EVMChainInfo(
				EVMChain.SLCCHAIN.toString(), 
				"SLCChain Mainnet", 
				288022981L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("SLCChain Mainnet Gas Token", "SLC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc1.sl.cool");
					this.add("https://rpc2.sl.cool");
					this.add("https://rpc3.sl.cool");
					this.add("https://rpc4.sl.cool");
					this.add("https://rpc5.sl.cool");
					this.add("https://rpc6.sl.cool");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.sl.cool");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/288022981");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.SLCCHAIN, slcchain);
		
		// dnachain
		EVMChainInfo dnachain = new EVMChainInfo(
				EVMChain.DNACHAIN.toString(), 
				"DNAChain Mainnet", 
				287022981L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("DNAChain Mainnet Gas Token", "DNA", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc1.gene.network");
					this.add("https://rpc2.gene.network");
					this.add("https://rpc3.gene.network");
					this.add("https://rpc4.gene.network");
					this.add("https://rpc5.gene.network");
					this.add("https://rpc6.gene.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.gene.network");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/287022981");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.DNACHAIN, dnachain);
		
		// istchain
		EVMChainInfo istchain = new EVMChainInfo(
				EVMChain.ISTCHAIN.toString(), 
				"ISTChain Mainnet", 
				286022981L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("ISTChain Mainnet Gas Token", "IST", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc1.istchain.org");
					this.add("https://rpc2.istchain.org");
					this.add("https://rpc3.istchain.org");
					this.add("https://rpc4.istchain.org");
					this.add("https://rpc5.istchain.org");
					this.add("https://rpc6.istchain.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.istchain.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/286022981");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ISTCHAIN, istchain);
		
		// splendor
		EVMChainInfo splendor = new EVMChainInfo(
				EVMChain.SPLENDOR.toString(), 
				"Splendor Mainnet", 
				2691L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Splendor Mainnet Gas Token", "SPLD", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet-rpc.splendor.org");
					this.add("https://splendor-rpc.org/");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.splendor.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/2691");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.SPLENDOR, splendor);
		
		// eadx
		EVMChainInfo eadx = new EVMChainInfo(
				EVMChain.EADX.toString(), 
				"EADX Network", 
				198724L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("EADX Network Gas Token", "EDX", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.eadx.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.eadx.network");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/198724");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.EADX, eadx);
		
		// incentiv
		EVMChainInfo incentiv = new EVMChainInfo(
				EVMChain.INCENTIV.toString(), 
				"Incentiv", 
				24101L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Incentiv Gas Token", "CENT", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.incentiv.io");
					this.add("https://rpc-archive.incentiv.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.incentiv.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/24101");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.INCENTIV, incentiv);
		
		// ulalo
		EVMChainInfo ulalo = new EVMChainInfo(
				EVMChain.ULALO.toString(), 
				"ULALO Mainnet", 
				237007L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("ULALO Mainnet Gas Token", "ULA", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://grpc.ulalo.xyz/ext/bc/2uN4Y9JHkLeAJK85Y48LExpNnEiepf7VoZAtmjnwDSZzpZcNig/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://tracehawk.ulalo.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/237007");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ULALO, ulalo);
		
		// codenekt
		EVMChainInfo codenekt = new EVMChainInfo(
				EVMChain.CODENEKT.toString(), 
				"CodeNekt Mainnet", 
				235235L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("CodeNekt Mainnet Gas Token", "CDK", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc-mainnet-codenekt-rl.cogitus.io/ext/bc/ZG7cT4B1u3y7piZ9CzfejnTKnNAoehcifbJWUwBqgyD3RuEqK/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer-codenekt-mainnet.cogitus.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/235235");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.CODENEKT, codenekt);
		
		// moca
		EVMChainInfo moca = new EVMChainInfo(
				EVMChain.MOCA.toString(), 
				"Moca Chain Mainnet", 
				2288L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Moca Chain Mainnet Gas Token", "MOCA", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.mocachain.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.mocachain.org");
				}},
				new ArrayList<String>() {{
					this.add("https://scan.mocachain.org/faucet");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/2288");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.MOCA, moca);
		
		// ibvm
		EVMChainInfo ibvm = new EVMChainInfo(
				EVMChain.IBVM.toString(), 
				"IBVM Mainnet", 
				2105L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("IBVM Mainnet Gas Token", "BTC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc-mainnet.ibvm.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://ibvmscan.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/2105");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.IBVM, ibvm);
		
		// juchain
		EVMChainInfo juchain = new EVMChainInfo(
				EVMChain.JUCHAIN.toString(), 
				"JuChain Mainnet", 
				210000L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("JuChain Mainnet Gas Token", "JU", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.juchain.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://juscan.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/210000");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.JUCHAIN, juchain);

		
		// kasplex
		EVMChainInfo kasplex = new EVMChainInfo(
				EVMChain.KASPLEX.toString(), 
				"Kasplex zkEVM Mainnet", 
				202555L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Kasplex zkEVM Mainnet Gas Token", "KAS", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://evmrpc.kasplex.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.kasplex.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/202555");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.KASPLEX, kasplex);
		
		// bridgeless
		EVMChainInfo bridgeless = new EVMChainInfo(
				EVMChain.BRIDGELESS.toString(), 
				"Bridgeless Mainnet", 
				13441L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Bridgeless Mainnet Gas Token", "BRIDGE", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://eth-rpc.node0.mainnet.bridgeless.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.mainnet.bridgeless.com/");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/13441");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.BRIDGELESS, bridgeless);
		
		// aria
		EVMChainInfo aria = new EVMChainInfo(
				EVMChain.ARIA.toString(), 
				"ARIA Chain", 
				134235L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("ARIA Chain Gas Token", "ARIA", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.ariascan.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.ariascan.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/134235");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ARIA, aria);
		
		// roonchain
		EVMChainInfo roonchain = new EVMChainInfo(
				EVMChain.ROONCHAIN.toString(), 
				"RoonChain Mainnet", 
				1314520L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("RoonChain Mainnet Gas Token", "ROON", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet-rpc.roonchain.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://mainnet.roonchain.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1314520");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ROONCHAIN, roonchain);
		
		// mitosis
		EVMChainInfo mitosis = new EVMChainInfo(
				EVMChain.MITOSIS.toString(), 
				"Mitosis", 
				124816L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Mitosis Gas Token", "MITO", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.mitosis.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://mitoscan.io/");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/124816");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.MITOSIS, mitosis);
		
		// nobody
		EVMChainInfo nobody = new EVMChainInfo(
				EVMChain.NOBODY.toString(), 
				"Nobody Mainnet", 
				123999L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Nobody Mainnet Gas Token", "IDS", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://a-rpc.nobody.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://a-scan.nobody.network");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/123999");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.NOBODY, nobody);
		
		// eco
		EVMChainInfo eco = new EVMChainInfo(
				EVMChain.ECO.toString(), 
				"ECO Mainnet", 
				12343L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("ECO Mainnet Gas Token", "ELA", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://api.elastos.io/eco");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://eco.elastos.io/");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/12343");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ECO, eco);
		
		// fitochain
		EVMChainInfo fitochain = new EVMChainInfo(
				EVMChain.FITOCHAIN.toString(), 
				"Fitochain", 
				1233L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Fitochain Gas Token", "FITO", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.fitochain.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.fitochain.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1233");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.FITOCHAIN, fitochain);
		
		// intuition
		EVMChainInfo intuition = new EVMChainInfo(
				EVMChain.INTUITION.toString(), 
				"Intuition Mainnet", 
				1155L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Intuition Mainnet Gas Token", "TRUST", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://intuition.calderachain.xyz/http");
					this.add("https://rpc.intuition.systems");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.intuition.systems");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1155");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.INTUITION, intuition);
		
		// grx
		EVMChainInfo grx = new EVMChainInfo(
				EVMChain.GRX.toString(), 
				"GRX Mainnet", 
				1110L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("GRX Mainnet Gas Token", "GRX", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.grxchain.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://grxscan.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1110");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.GRX, grx);
		
		// realchain
		EVMChainInfo realchain = new EVMChainInfo(
				EVMChain.REALCHAIN.toString(), 
				"RealChain Mainnet", 
				1098L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("RealChain Mainnet Gas Token", "R", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.realchain.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.realchain.io/");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1098");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.REALCHAIN, realchain);
		
		// ozone
		EVMChainInfo ozone = new EVMChainInfo(
				EVMChain.OZONE.toString(), 
				"Ozone Mainnet", 
				10121L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Ozone Mainnet Gas Token", "OZONE", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://chain.ozonescan.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://ozonescan.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/10121");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.OZONE, ozone);
		
		// qiemainnet
		EVMChainInfo qiemainnet = new EVMChainInfo(
				EVMChain.QIEMAINNET.toString(), 
				"QIEMainnet", 
				1990L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("QIEMainnet Gas Token", "QIE", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc1mainnet.qie.digital");
					this.add("https://rpc5mainnet.qie.digital");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://mainnet.qie.digital/");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1990");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.QIEMAINNET, qiemainnet);
		
		// mova
		EVMChainInfo movab = new EVMChainInfo(
				EVMChain.MOVABETA.toString(), 
				"Mova Beta", 
				10323L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Mova Beta Gas Token", "MARS", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mars.rpc.movachain.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.mars.movachain.com");
				}},
				new ArrayList<String>() {{
					this.add("https://faucet.mars.movachain.com");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/10323");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.MOVABETA, movab);
		
		// mova
		EVMChainInfo mova = new EVMChainInfo(
				EVMChain.MOVA.toString(), 
				"Mova Mainnet", 
				61900L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Mova Mainnet Gas Token", "MOVA", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.movachain.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.movachain.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/61900");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.MOVA, mova);
		

		// autonity
		EVMChainInfo autonity = new EVMChainInfo(
				EVMChain.AUTONITY.toString(), 
				"Autonity Mainnet", 
				65000000L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Autonity Mainnet Gas Token", "ATN", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://autonity.rpc.web3cdn.network");
					this.add("https://autonity.rpc.subquery.network/public");
					this.add("https://rpc.autonity-apis.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://autonityscan.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/65000000");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.AUTONITY, autonity);
		
		// sovra
		EVMChainInfo sovra = new EVMChainInfo(
				EVMChain.SOVRA.toString(), 
				"Sovra", 
				65536001L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Sovra Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.sovra.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.sovra.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/65536001");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.SOVRA, sovra);
		
		// jmdt
		EVMChainInfo jmdt = new EVMChainInfo(
				EVMChain.JMDT.toString(), 
				"JMDT Mainnet", 
				7000700L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("JMDT Mainnet Gas Token", "JMDT", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.jmdt.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.jmdt.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/7000700");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.JMDT, jmdt);
		
		// vrcn
		EVMChainInfo vrcn = new EVMChainInfo(
				EVMChain.VRCN.toString(), 
				"VRCN Chain Mainnet", 
				7131L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("VRCN Chain Mainnet Gas Token", "VRCN", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc-mainnet-4.vrcchain.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://dxb.vrcchain.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/7131");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.VRCN, vrcn);
		
		// carrchain
		EVMChainInfo carrchain = new EVMChainInfo(
				EVMChain.CARRCHAIN.toString(), 
				"CarrChain Mainnet", 
				7667L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("CarrChain Mainnet Gas Token", "CARR", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.carrchain.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://carrscan.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/7667");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.CARRCHAIN, carrchain);
		
		// onyx
		EVMChainInfo onyx = new EVMChainInfo(
				EVMChain.ONYX.toString(), 
				"Onyx", 
				80888L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Onyx Gas Token", "XCN", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.onyx.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.onyx.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/80888");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ONYX, onyx);
		
		// forknet
		EVMChainInfo forknet = new EVMChainInfo(
				EVMChain.FORKNET.toString(), 
				"Forknet", 
				8338L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Forknet Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc-forknet.t.conduit.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://forkscan.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/8338");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.FORKNET, forknet);

		// binaryholdings
		EVMChainInfo binaryholdings = new EVMChainInfo(
				EVMChain.BINARYHOLDINGS.toString(), 
				"BinaryHoldings Mainnet", 
				836L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("BinaryHoldings Mainnet Gas Token", "BNRY", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc-binaryholdings.cogitus.io/ext/bc/J3MYb3rDARLmB7FrRybinyjKqVTqmerbCr9bAXDatrSaHiLxQ/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer-binaryholdings.cogitus.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/836");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.BINARYHOLDINGS, binaryholdings);
		
		// codex
		EVMChainInfo codex = new EVMChainInfo(
				EVMChain.CODEX.toString(), 
				"Codex Mainnet", 
				81224L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Codex Mainnet Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.codex.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.codex.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/81224");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.CODEX, codex);

		
		// capx
		EVMChainInfo capx = new EVMChainInfo(
				EVMChain.CAPX.toString(), 
				"CAPX", 
				757L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("CAPX Gas Token", "CAPX", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://capx-mainnet.calderachain.xyz/http");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://capxscan.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/757");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.CAPX, capx);
		
		// growfitter
		EVMChainInfo growfitter = new EVMChainInfo(
				EVMChain.GROWFITTER.toString(), 
				"Growfitter Mainnet", 
				7084L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Growfitter Mainnet Gas Token", "GFIT", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc-mainnet-growfitter-rl.cogitus.io/ext/bc/2PdUCtQocNDvbVWy8ch4PdaicTHA2h5keHLAAPcs9Pr8tYaUg3/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer-growfitter-mainnet.cogitus.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/7084");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.GROWFITTER, growfitter);

		
		// gate
		EVMChainInfo gate = new EVMChainInfo(
				EVMChain.GATE.toString(), 
				"Gate Layer", 
				10088L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Gate Layer Gas Token", "GT", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://gatelayer-mainnet.gatenode.cc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://www.gatescan.org/gatelayer");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/10088");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.GATE, gate);
		
		// ethwmainnet
		EVMChainInfo ethwmainnet = new EVMChainInfo(
				EVMChain.ETHWMAINNET.toString(), 
				"ETHW-mainnet", 
				10001L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("ETHW-mainnet Gas Token", "ETHW", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet.ethereumpow.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://www.oklink.com/ethw/");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/10001");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ETHWMAINNET, ethwmainnet);
		
		// watr
		EVMChainInfo watr = new EVMChainInfo(
				EVMChain.WATR.toString(), 
				"Watr Mainnet", 
				192L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Watr Mainnet Gas Token", "WAT", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.watr.org/ext/bc/EypLFUSzC2wdbFJovYS3Af1E7ch1DJf7KxKoGR5QFPErxQkG1/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.watr.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/192");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.WATR, watr);
		
		// epix
		EVMChainInfo epix = new EVMChainInfo(
				EVMChain.EPIX.toString(), 
				"Epix", 
				1916L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Epix Gas Token", "EPIX", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://evmrpc.epix.zone");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("http://scan.epix.zone/");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1916");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.EPIX, epix);
		
		// hpp
		EVMChainInfo hpp = new EVMChainInfo(
				EVMChain.HPP.toString(), 
				"HPP Mainnet", 
				190415L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("HPP Mainnet Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet.hpp.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.hpp.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/190415");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.HPP, hpp);
		
		// gomchain
		EVMChainInfo gomchain = new EVMChainInfo(
				EVMChain.GOMCHAIN.toString(), 
				"GomChain Mainnet", 
				190278L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("GomChain Mainnet Gas Token", "GOM", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.gomchain.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.gomchain.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/190278");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.GOMCHAIN, gomchain);
		
		// lit
		EVMChainInfo lit = new EVMChainInfo(
				EVMChain.LIT.toString(), 
				"Lit Chain Mainnet", 
				175200L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Lit Chain Mainnet Gas Token", "LITKEY", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://lit-chain-rpc.litprotocol.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://lit-chain-explorer.litprotocol.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/175200");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.LIT, lit);
		
		// nomina
		EVMChainInfo nomina = new EVMChainInfo(
				EVMChain.NOMINA.toString(), 
				"Nomina", 
				166L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Nomina Gas Token", "NOM", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet.nomina.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://omniscan.network");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/166");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.NOMINA, nomina);
		
		// waba
		EVMChainInfo waba = new EVMChainInfo(
				EVMChain.WABA.toString(), 
				"WABA Chain Mainnet", 
				327126L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("WABA Chain Mainnet Gas Token", "WABA", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.wabaworld.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.wabaworld.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/327126");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.WABA, waba);
		
		// sova
		EVMChainInfo sova = new EVMChainInfo(
				EVMChain.SOVA.toString(), 
				"Sova", 
				100021L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Sova Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.sova.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.sova.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/100021");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.SOVA, sova);
		
		// doma
		EVMChainInfo doma = new EVMChainInfo(
				EVMChain.DOMA.toString(), 
				"Doma", 
				97477L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Doma Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.doma.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.doma.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/97477");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.DOMA, doma);
		
		// hyperagi
		EVMChainInfo hyperagi = new EVMChainInfo(
				EVMChain.HYPERAGI.toString(), 
				"HyperAGI Mainnet", 
				2868L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("HyperAGI Mainnet Gas Token", "HYPT", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.hyperagi.network");
					this.add("https://rpc.hyperagi.ai");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://hyperscan.hyperagi.ai");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/2868");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.HYPERAGI, hyperagi);
		
		// haust
		EVMChainInfo haust = new EVMChainInfo(
				EVMChain.HAUST3864.toString(), 
				"Haust Network", 
				3864L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Haust Network Gas Token", "HAUST", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://haust-network-rpc.eu-north-2.gateway.fm");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://haustscan.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/3864");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.HAUST3864, haust);
		
		// dcomm
		EVMChainInfo dcomm = new EVMChainInfo(
				EVMChain.DCOMM.toString(), 
				"DComm Mainnet", 
				326663L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("DComm Mainnet Gas Token", "DCM", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc-mainnet-dcomm-rl.cogitus.io/ext/bc/2QJ6d1ue6UyXNXrMdGnELFc2AjMdMqs8YbX3sT3k4Nin2RcWSm/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer-dcomm.cogitus.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/326663");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.DCOMM, dcomm);
		
		// entropy
		EVMChainInfo entropy = new EVMChainInfo(
				EVMChain.ENTROPY.toString(), 
				"Entropy Chain", 
				19911L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Entropy Chain Gas Token", "KGFe", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc-mainnet.entropychain.live");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.entropychain.live");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/19911");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ENTROPY, entropy);
		
		// dinari
		EVMChainInfo dinari = new EVMChainInfo(
				EVMChain.DINARI.toString(), 
				"Dinari Financial Network", 
				202110L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Dinari Financial Network Gas Token", "DGAS", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://subnets.avax.network/dinari/mainnet/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://subnets.avax.network/dinari");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/202110");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.DINARI, dinari);
		
		// h2
		EVMChainInfo h2o = new EVMChainInfo(
				EVMChain.H2.toString(), 
				"H2 Chain Mainnet", 
				2582L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("H2 Chain Mainnet Gas Token", "H2", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.h2chain.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://h2scan.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/2582");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.H2, h2o);
		
		// c9xchain
		EVMChainInfo c9xchain = new EVMChainInfo(
				EVMChain.C9XCHAIN.toString(), 
				"C9XChain", 
				3230L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("C9XChain Gas Token", "CXC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://services.tanssi-mainnet.network/tanssi-2002");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://blockscan-tanssi.c9tech.com.br");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/3230");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.C9XCHAIN, c9xchain);

		
		// h2
		EVMChainInfo h2 = new EVMChainInfo(
				EVMChain.H2LAMBDA.toString(), 
				"H2 Lambda", 
				25821L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("H2 Lambda Gas Token", "H2", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.h-1.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://lambda.h2scan.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/25821");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.H2LAMBDA, h2);
		
		// btb
		EVMChainInfo btb = new EVMChainInfo(
				EVMChain.BTB.toString(), 
				"BTB Chain Mainnet", 
				509L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("BTB Chain Mainnet Gas Token", "BTB", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://btb-dataseed.btbchain.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://btb-explorer.btbchain.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/509");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.BTB, btb);
		
		// eto
		EVMChainInfo eto = new EVMChainInfo(
				EVMChain.ETO.toString(), 
				"ETO L1", 
				69670L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("ETO L1 Gas Token", "eGAS", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://eto.ash.center/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://eto-explorer.ash.center");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/69670");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ETO, eto);
		
		// makachain
		EVMChainInfo makachain = new EVMChainInfo(
				EVMChain.MAKACHAIN.toString(), 
				"MakaChain Mainnet", 
				777178L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("MakaChain Mainnet Gas Token", "MFUEL", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://api.makachain.io/ext/bc/2TfGh1pyDoumsDgTxDmWoseRD7GGRwrpX19HLsQdeCRqwK9BqZ/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://makascan.io/");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/777178");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.MAKACHAIN, makachain);
		
		// stable
		EVMChainInfo stable = new EVMChainInfo(
				EVMChain.STABLE.toString(), 
				"Stable Mainnet", 
				988L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Stable Mainnet Gas Token", "gUSDT", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.stable.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://stablescan.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/988");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.STABLE, stable);
		
		// hyperx
		EVMChainInfo hyperx = new EVMChainInfo(
				EVMChain.HYPERX.toString(), 
				"HyperX", 
				80808L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("HyperX Gas Token", "HPX", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.hyperx.technology");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.hyperx.technology");
				}},
				new ArrayList<String>() {{
					this.add("https://faucet.hyperx.technology");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/80808");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.HYPERX, hyperx);
		
		// apollo
		EVMChainInfo apollo = new EVMChainInfo(
				EVMChain.APOLLO.toString(), 
				"Apollo Mainnet", 
				62606L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Apollo Mainnet Gas Token", "APOLLO", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet-rpc.apolloscan.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://apolloscan.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/62606");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.APOLLO, apollo);
		
		// silverbitcoin
		EVMChainInfo silverbitcoin = new EVMChainInfo(
				EVMChain.SILVERBITCOIN.toString(), 
				"SilverBitcoin", 
				5200L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("SilverBitcoin Gas Token", "SBTC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.silverbitcoin.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.silverbitcoin.org/");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/5200");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.SILVERBITCOIN, silverbitcoin);
		
		// tensora
		EVMChainInfo tensora = new EVMChainInfo(
				EVMChain.TENSORA.toString(), 
				"Tensora", 
				44444444L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Tensora Gas Token", "BNB", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.tensora.sh");
					this.add("http://63.250.32.66:8545");

				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.tensora.sh");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/44444444");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.TENSORA, tensora);
		
		// virbicoin
		EVMChainInfo virbicoin = new EVMChainInfo(
				EVMChain.VIRBICOIN.toString(), 
				"VirBiCoin", 
				329L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("VirBiCoin Gas Token", "VBC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.digitalregion.jp");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.digitalregion.jp");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/329");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.VIRBICOIN, virbicoin);
		
		// alpha
		EVMChainInfo alpha = new EVMChainInfo(
				EVMChain.ALPHA.toString(), 
				"Alpha Chain Mainnet", 
				3111L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Alpha Chain Mainnet Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.goalpha.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.goalpha.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/3111");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ALPHA, alpha);
		
		// buburuza
		EVMChainInfo buburuza = new EVMChainInfo(
				EVMChain.BUBURUZA.toString(), 
				"Buburuza Mainnet", 
				30143370385L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Buburuza Mainnet Gas Token", "BUB", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc-mainnet.buburuza.com/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer-mainnet.buburuza.com/");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/30143370385");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.BUBURUZA, buburuza);
		
		// notes
		EVMChainInfo notes = new EVMChainInfo(
				EVMChain.NOTES.toString(), 
				"Notes Network Mainnet", 
				2643L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Notes Network Mainnet Gas Token", "KTO", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc-mainnet.noschain.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://nosscan.noschain.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/2643");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.NOTES, notes);
		
		// h2
		EVMChainInfo h22 = new EVMChainInfo(
				EVMChain.H22582.toString(), 
				"H2", 
				2582L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("H2 Gas Token", "H2", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.h2chain.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://h2scan.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/2582");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.H22582, h22);
		
		// dinari
		EVMChainInfo dinari2 = new EVMChainInfo(
				EVMChain.DINARI179205.toString(), 
				"Dinari Financial Network Paper", 
				179205L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Dinari Financial Network Paper Gas Token", "DGAS", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://subnets.avax.network/dfnpaper/testnet/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://subnets-test.avax.network/dfnpaper");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/179205");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.DINARI179205, dinari2);
		
		// injective
		EVMChainInfo injective = new EVMChainInfo(
				EVMChain.INJECTIVE.toString(), 
				"Injective", 
				1776L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Injective Gas Token", "INJ", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://sentry.evm-rpc.injective.network");
					this.add("https://injectiveevm-rpc.polkachu.com");
					this.add("https://injective-evm-rpc.scvsecurity.io");

				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://blockscout.injective.network");
				}},
				new ArrayList<String>() {{
					this.add("https://injective.com/getinj");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1776");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.INJECTIVE, injective);
		
		EVMChainInfo aipaw = new EVMChainInfo(
				EVMChain.AIPAW.toString(), 
				"AIPaw Mainnet", 
				90025L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("AIPaw Mainnet Gas Token", "AIPAW", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.aipaw.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/90025");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.AIPAW, aipaw);

		// kubl2
		EVMChainInfo kubl2 = new EVMChainInfo(
				EVMChain.KUBL2.toString(), 
				"KUB Layer 2 Mainnet", 
				9601L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("KUB Layer 2 Mainnet Gas Token", "KUB", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://kublayer2.kubchain.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://kublayer2.kubscan.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/9601");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.KUBL2, kubl2);
		
		// horizen
		EVMChainInfo horizen = new EVMChainInfo(
				EVMChain.HORIZEN.toString(), 
				"Horizen Mainnet", 
				26514L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Horizen Mainnet Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://horizen.calderachain.xyz/http");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://horizen.calderaexplorer.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/26514");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.HORIZEN, horizen);
		
		// monsoon
		EVMChainInfo monsoon = new EVMChainInfo(
				EVMChain.MONSOONALPHA.toString(), 
				"Monsoon Alpha", 
				100611L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Monsoon Alpha Gas Token", "RDL", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://alpha.monsoon.rainfall.one");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scout.alpha.monsoon.rainfall.one");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/100611");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.MONSOONALPHA, monsoon);
		
		// cycle
		EVMChainInfo cycle = new EVMChainInfo(
				EVMChain.CYCLE.toString(), 
				"Cycle Network Mainnet Frigate", 
				88688L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Cycle Network Mainnet Frigate Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://frigate-rpc-mainnet.cyclenetwork.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/88688");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.CYCLE, cycle);
		
		// cpchain
		EVMChainInfo cpchain = new EVMChainInfo(
				EVMChain.CPCHAIN.toString(), 
				"CpChain", 
				86608L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("CpChain Gas Token", "CP", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.cpchain.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.cpchain.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/86608");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.CPCHAIN, cpchain);
		
		// rivool
		EVMChainInfo rivool = new EVMChainInfo(
				EVMChain.RIVOOL.toString(), 
				"Rivool", 
				28540L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Rivool Gas Token", "RVO", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://services.tanssi-mainnet.network/tanssi-2005");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://evmexplorer.tanssi-chains.network/?rpcUrl=https://services.tanssi-mainnet.network/tanssi-2005");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/28540");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.RIVOOL, rivool);

		
		// gotas
		EVMChainInfo gotas = new EVMChainInfo(
				EVMChain.GOTAS.toString(), 
				"Gotas Social", 
				23232L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Gotas Social Gas Token", "GOTAS", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://services.tanssi-mainnet.network/tanssi-2006");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://evmexplorer.tanssi-chains.network/?rpcUrl=https://services.tanssi-mainnet.network/tanssi-2006");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/23232");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.GOTAS, gotas);
		
		// trexx
		EVMChainInfo trexx = new EVMChainInfo(
				EVMChain.TREXX.toString(), 
				"Trexx", 
				13766L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Trexx Gas Token", "TRX", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://services.tanssi-mainnet.network/tanssi-2003");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://evmexplorer.tanssi-chains.network/?rpcUrl=https://services.tanssi-mainnet.network/tanssi-2003");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/13766");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.TREXX, trexx);
		
		// qubetics
		EVMChainInfo qubetics = new EVMChainInfo(
				EVMChain.QUBETICS.toString(), 
				"Qubetics Mainnet", 
				9030L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Qubetics Mainnet Gas Token", "TICS", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.qubetics.com");
					this.add("https://evm-rpc-arch.qubetics.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://ticsscan.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/9030");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.QUBETICS, qubetics);
		
		// quai
		EVMChainInfo quai = new EVMChainInfo(
				EVMChain.QUAI.toString(), 
				"Quai Network Mainnet", 
				9L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Quai Network Mainnet Gas Token", "QUAI", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.quai.network/cyprus1");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://quaiscan.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/9");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.QUAI, quai);
		
		// jovay
		EVMChainInfo jovay = new EVMChainInfo(
				EVMChain.JOVAY.toString(), 
				"Jovay Mainnet", 
				5734951L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Jovay Mainnet Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://api.zan.top/public/jovay-mainnet");
					this.add("https://rpc.jovay.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.jovay.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/5734951");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.JOVAY, jovay);
		
		// studio
		EVMChainInfo studio1 = new EVMChainInfo(
				EVMChain.STUDIO240241.toString(), 
				"Studio Blockchain Mainnet 240241", 
				240241L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Studio Blockchain Mainnet Gas Token", "STO", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet.studio-blockchain.com");
					this.add("https://mainnet2.studio-blockchain.com");
					this.add("https://mainnet3.studio-blockchain.com");
					this.add("https://mainnet.studio-scan.com");
					this.add("https://mainnet2.studio-scan.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://studio-scan.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/240241");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.STUDIO240241, studio1);

		// studio
		EVMChainInfo studio = new EVMChainInfo(
				EVMChain.STUDIO616.toString(), 
				"Studio Chain", 
				616L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Studio Chain Gas Token", "KARRAT", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://studiochain-cf4a1621.calderachain.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://studiochain-cf4a1621.calderaexplorer.xyz/");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/616");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.STUDIO616, studio);
		
		// ZEROg
		EVMChainInfo ZEROg = new EVMChainInfo(
				EVMChain.ZEROG.toString(), 
				"0G Mainnet", 
				16661L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("0G Mainnet Gas Token", "0G", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://evmrpc.0g.ai");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/16661");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ZEROG, ZEROg);
		
		// mantrachain
		EVMChainInfo mantrachain = new EVMChainInfo(
				EVMChain.MANTRACHAIN.toString(), 
				"MANTRACHAIN Mainnet", 
				5888L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("MANTRACHAIN Mainnet Gas Token", "OM", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://evm.mantrachain.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("http://mantrascan.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/5888");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.MANTRACHAIN, mantrachain);
		
		// worldmobilechainmainnet
		EVMChainInfo worldmobilechainmainnet = new EVMChainInfo(
				EVMChain.WORLDMOBILECHAINMAINNET.toString(), 
				"WorldMobileChain-Mainnet", 
				869L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("WorldMobileChain-Mainnet Gas Token", "WMTX", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://worldmobilechain-mainnet.g.alchemy.com/public");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.worldmobile.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/869");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.WORLDMOBILECHAINMAINNET, worldmobilechainmainnet);
		
		// galactica
		EVMChainInfo galactica = new EVMChainInfo(
				EVMChain.GALACTICA.toString(), 
				"Galactica Mainnet", 
				613419L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Galactica Mainnet Gas Token", "GNET", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://galactica-mainnet.g.alchemy.com/public");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.galactica.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/613419");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.GALACTICA, galactica);
		
		// cross
		EVMChainInfo cross = new EVMChainInfo(
				EVMChain.CROSS.toString(), 
				"CROSS Mainnet", 
				612055L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("CROSS Mainnet Gas Token", "CROSS", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet.crosstoken.io:22001");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://www.crossscan.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/612055");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.CROSS, cross);
		
		// yeying
		EVMChainInfo yeying = new EVMChainInfo(
				EVMChain.YEYING.toString(), 
				"YeYing Network", 
				5432L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("YeYing Network Gas Token", "YYT", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://blockchain.yeying.pub");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://blockscout.yeying.pub");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/5432");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.YEYING, yeying);

		
		// loot
		EVMChainInfo loot = new EVMChainInfo(
				EVMChain.LOOT.toString(), 
				"Loot Mainnet", 
				5151706L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Loot Mainnet Gas Token", "AGLD", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.lootchain.com/http");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.lootchain.com/");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/5151706");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.LOOT, loot);
		
		// syndicate
		EVMChainInfo syndicatec = new EVMChainInfo(
				EVMChain.SYNDICATECOMMONS.toString(), 
				"Syndicate Commons", 
				510003L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Syndicate Commons Gas Token", "SYND", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://commons.rpc.syndicate.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.commons.syndicate.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/510003");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.SYNDICATECOMMONS, syndicatec);
		
		// syndicate
		EVMChainInfo syndicate = new EVMChainInfo(
				EVMChain.SYNDICATE.toString(), 
				"Syndicate Mainnet", 
				510L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Syndicate Mainnet Gas Token", "SYND", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.syndicate.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.syndicate.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/510");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.SYNDICATE, syndicate);
		
		// plasma
		EVMChainInfo plasma = new EVMChainInfo(
				EVMChain.PLASMA.toString(), 
				"Plasma Mainnet", 
				9745L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Plasma Mainnet Gas Token", "XPL", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.plasma.to");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://plasmascan.to");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/9745");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.PLASMA, plasma);
		
		// openledger
		EVMChainInfo openledger = new EVMChainInfo(
				EVMChain.OPENLEDGER.toString(), 
				"OpenLedger Mainnet", 
				1612L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("OpenLedger Mainnet Gas Token", "OPEN", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.openledger.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.openledger.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1612");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.OPENLEDGER, openledger);
		
		// humanity
		EVMChainInfo humanity = new EVMChainInfo(
				EVMChain.HUMANITY.toString(), 
				"Humanity Protocol", 
				6985385L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Humanity Protocol Gas Token", "H", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://humanity-mainnet.g.alchemy.com/public");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://humanity-mainnet.explorer.alchemy.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/6985385");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.HUMANITY, humanity);
		
		// scenium
		EVMChainInfo scenium = new EVMChainInfo(
				EVMChain.SCENIUM.toString(), 
				"Scenium", 
				335700L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Scenium Gas Token", "SCEN", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://services.tanssi-mainnet.network/tanssi-2004");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://evmexplorer.tanssi-chains.network/?rpcUrl=https://services.tanssi-mainnet.network/tanssi-2004");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/335700");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.SCENIUM, scenium);
		
		// monsoon
		EVMChainInfo monsoon2 = new EVMChainInfo(
				EVMChain.MONSOON.toString(), 
				"Monsoon ", 
				100610L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Monsoon  Gas Token", "RDP", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://monsoon.rainfall.one");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scout.monsoon.rainfall.one");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/100610");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.MONSOON, monsoon2);
		
		// pay1
		EVMChainInfo pay1 = new EVMChainInfo(
				EVMChain.PAY1.toString(), 
				"Pay1 Network", 
				98964L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Pay1 Network Gas Token", "Pay1", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.pay1coin.com");
					this.add("https://rpc0.pay1coin.com");
					this.add("https://rpc1.pay1coin.com");
					this.add("https://rpc2.pay1coin.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://pay1scan.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/98964");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.PAY1, pay1);
		
		// polkadot
		EVMChainInfo polkadot = new EVMChainInfo(
				EVMChain.POLKADOT.toString(), 
				"Polkadot", 
				420420419L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Polkadot Gas Token", "DOT", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://services.polkadothub-rpc.com/mainnet");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://polkadot.routescan.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/420420419");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.POLKADOT, polkadot);
		
		// transparency
		EVMChainInfo transparency = new EVMChainInfo(
				EVMChain.TRANSPARENCY.toString(), 
				"Transparency Solution", 
				179170L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Transparency Solution Gas Token", "CLT", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc-api.transparency.solutions");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://blockchain.transparency.solutions");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/179170");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.TRANSPARENCY, transparency);
		
		// bokuto
		EVMChainInfo bokuto = new EVMChainInfo(
				EVMChain.BOKUTO.toString(), 
				"bokuto", 
				737373L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("bokuto Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc-bokuto.katanarpc.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer-bokuto.katanarpc.com");
				}},
				new ArrayList<String>() {{
					this.add("https://faucet.katana.tools");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/737373");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.BOKUTO, bokuto);
		
		// katana
		EVMChainInfo katana = new EVMChainInfo(
				EVMChain.KATANA.toString(), 
				"katana", 
				747474L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("katana Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.katana.network");
					this.add("https://katana.gateway.tenderly.co");
					this.add("https://rpc.katanarpc.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.katanarpc.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/747474");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.KATANA, katana);
		
		// webchain
		EVMChainInfo webchain = new EVMChainInfo(
				EVMChain.WEBCHAIN.toString(), 
				"WebChain ETK", 
				1000001L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("WebChain ETK Gas Token", "ETK", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.webchain.e-talk.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://e-talk.xyz/webchain");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1000001");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.WEBCHAIN, webchain);
		
		// kiichain
		EVMChainInfo kiichain = new EVMChainInfo(
				EVMChain.KIICHAIN.toString(), 
				"KiiChain", 
				1783L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("KiiChain Gas Token", "KII", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://json-rpc.kiivalidator.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.kiichain.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1783");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.KIICHAIN, kiichain);

		
		// citrea
		EVMChainInfo citrea = new EVMChainInfo(
				EVMChain.CITREA.toString(), 
				"Citrea Mainnet", 
				4114L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Citrea Mainnet Gas Token", "cBTC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.mainnet.citrea.xyz/");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.mainnet.citrea.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/4114");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.CITREA, citrea);
		
		// yuuchain
		EVMChainInfo yuuchain = new EVMChainInfo(
				EVMChain.YUUCHAIN.toString(), 
				"YuuChain", 
				19001L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("YuuChain Gas Token", "YUU", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.theyuusystem.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.theyuusystem.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/19001");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.YUUCHAIN, yuuchain);
		
		// avi
		EVMChainInfo avi = new EVMChainInfo(
				EVMChain.AVI.toString(), 
				"AVI Coin", 
				963369L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("AVI Coin Gas Token", "AVI", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.avicoin.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.avicoin.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/963369");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.AVI, avi);
		
		// nillion
		EVMChainInfo nillion = new EVMChainInfo(
				EVMChain.NILLION.toString(), 
				"Nillion Network", 
				98875L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Nillion Network Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.nillion.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.nillion.network");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/98875");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.NILLION, nillion);
		
		// rome
		EVMChainInfo rome = new EVMChainInfo(
				EVMChain.ROMEPALATINE.toString(), 
				"Rome Palatine", 
				7531L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Rome Palatine Gas Token", "RSOL", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://palatine.romeprotocol.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://romescout-palatine.romeprotocol.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/7531");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ROMEPALATINE, rome);
		
		// rome
		EVMChainInfo rome2 = new EVMChainInfo(
				EVMChain.AVENTINEROME.toString(), 
				"Rome Mainnet 0 Aventine", 
				7532L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Rome Mainnet 0 Aventine Gas Token", "RSOL", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://aventine-i.mainnet-0.romeprotocol.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://romescout-aventine-i.mainnet-0.romeprotocol.xyz");
				}},
				new ArrayList<String>() {{
					this.add("https://deposit.mainnet-0.romeprotocol.xyz");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/7532");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.AVENTINEROME, rome2);
		
		// rome
		EVMChainInfo romep = new EVMChainInfo(
				EVMChain.PALATINEROME.toString(), 
				"Rome Mainnet 0 Palatine", 
				7531L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Rome Mainnet 0 Palatine Gas Token", "RSOL", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://palatine-i.mainnet-0.romeprotocol.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://romescout-palatine-i.mainnet-0.romeprotocol.xyz");
				}},
				new ArrayList<String>() {{
					this.add("https://deposit.mainnet-0.romeprotocol.xyz");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/7531");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.PALATINEROME, romep);
		
		// nexera
		EVMChainInfo nexera = new EVMChainInfo(
				EVMChain.NEXERA.toString(), 
				"Nexera Mainnet", 
				7208L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Nexera Mainnet Gas Token", "NXRA", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.nexera.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/7208");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.NEXERA, nexera);
		
		// ganchain
		EVMChainInfo ganchain = new EVMChainInfo(
				EVMChain.GANCHAIN.toString(), 
				"GANchain L1", 
				4048L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("GANchain L1 Gas Token", "GPU", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.gpu.net");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://ganscan.gpu.net");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/4048");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.GANCHAIN, ganchain);
		
		// buycex
		EVMChainInfo buycex = new EVMChainInfo(
				EVMChain.BUYCEX.toString(), 
				"BuyCex Infinity Chain", 
				323L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("BuyCex Infinity Chain Gas Token", "BCX", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.buycex.net");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://buycex.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/323");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.BUYCEX, buycex);
		
		// nutriemp
		EVMChainInfo nutriemp = new EVMChainInfo(
				EVMChain.NUTRIEMP.toString(), 
				"NutriEmp Chain", 
				432L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("NutriEmp Chain Gas Token", "GRAMZ", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.nutriemp-chain.link");
					this.add("https://rpc.nutriemp.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.nutriemp-chain.link");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/432");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.NUTRIEMP, nutriemp);
		
		// eni
				EVMChainInfo eni = new EVMChainInfo(
						EVMChain.ENI.toString(), 
						"ENI Mainnet", 
						173L, 
						BlockchainType.PUBLIC.toString(),
						new EVMCurrency("ENI Mainnet Gas Token", "ENI", 18), 
						EVMPriceMechanism.LEGACY.toString(),30,
						"30000000000", //30 gwei
						null, // enforced min gasprice
						"300000", //300k units
						new ArrayList<String>() {{
							this.add("https://rpc.eniac.network");
							this.add("https://rpc1.eniac.network");
							this.add("https://rpc2.eniac.network");
							this.add("https://enirpc.com");
							this.add("https://jp.enirpc.com");
							this.add("https://jp.eniacrpc.net");
						}}, 
						new ArrayList<String>(), // archive nodes (if available)
						new ArrayList<String>(),
						new ArrayList<String>(),
						new ArrayList<String>() {{
							this.add("https://scan.eniac.network");
						}},
						new ArrayList<String>() {{
							this.add("<TODO>");
						}},
						new ArrayList<String>() {{
							this.add("https://chainlist.org/chain/173");
						}},
						BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
						BlockchainTokenDetailsEVM.generateDummyNFTIndex());
				networks.put(EVMChain.ENI, eni);
		
		// deinfra
		EVMChainInfo deinfra = new EVMChainInfo(
				EVMChain.DEINFRA.toString(), 
				"DeInfra Mainnet", 
				100501L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("DeInfra Mainnet Gas Token", "SK", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://c100501n3.deinfra.net:443/jsonrpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.deinfra.net");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/100501");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.DEINFRA, deinfra);
		
		// botanix
		EVMChainInfo botanix = new EVMChainInfo(
				EVMChain.BOTANIX.toString(), 
				"Botanix Mainnet", 
				3637L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Botanix Mainnet Gas Token", "BTC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.botanixlabs.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://botanixscan.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/3637");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.BOTANIX, botanix);
		
		// reddio
		EVMChainInfo reddio = new EVMChainInfo(
				EVMChain.REDDIO.toString(), 
				"Reddio", 
				50342L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Reddio Gas Token", "RDO", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet.reddio.com/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://reddio.cloud.blockscout.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/50342");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.REDDIO, reddio);
		
		// althea
		EVMChainInfo althea = new EVMChainInfo(
				EVMChain.ALTHEA.toString(), 
				"Althea L1 Mainnet", 
				258432L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Althea L1 Mainnet Gas Token", "ALTHEA", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.althea.zone");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://mintscan.io/althea");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/258432");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ALTHEA, althea);
		
		// realio
		EVMChainInfo realio = new EVMChainInfo(
				EVMChain.REALIO.toString(), 
				"Realio", 
				3301L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Realio Gas Token", "RIO", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://json-rpc.realio.network");
					this.add("https://realio.json-rpc.decentrio.ventures");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.realio.network");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/3301");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.REALIO, realio);
		
		// tac
		EVMChainInfo tac = new EVMChainInfo(
				EVMChain.TAC.toString(), 
				"TAC Mainnet", 
				239L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("TAC Mainnet Gas Token", "TAC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.tac.build");
					this.add("https://rpc.ankr.com/tac");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://tac.blockscout.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/239");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.TAC, tac);

		// edge
		EVMChainInfo edge = new EVMChainInfo(
				EVMChain.EDGE.toString(), 
				"Edge", 
				3343L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Edge Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://edge-mainnet.g.alchemy.com/public");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://edge-mainnet.explorer.alchemy.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/3343");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.EDGE, edge);
		
		// layer
		EVMChainInfo layer = new EVMChainInfo(
				EVMChain.LAYER.toString(), 
				"Layer Edge Mainnet", 
				4207L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Layer Edge Mainnet Gas Token", "EDGEN", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://layeredge-mainnet-evm.itrocket.net");
					this.add("https://layeredge.rpc.subquery.network/public");
					this.add("https://rpc.layeredge.io");
					this.add("https://rpc2.layeredge.io");
					this.add("https://rpc.layeredge.foundation");
					this.add("https://rpc2.layeredge.foundation");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://edgenscan.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/4207");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.LAYER, layer);
		
		// eteria
		EVMChainInfo eteria = new EVMChainInfo(
				EVMChain.ETERIA.toString(), 
				"Eteria Mainnet", 
				140L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Eteria Mainnet Gas Token", "ERA", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet.eternalcoin.io/v1");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/140");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ETERIA, eteria);
		
		// r5
		EVMChainInfo r5 = new EVMChainInfo(
				EVMChain.R5.toString(), 
				"R5 Network", 
				337L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("R5 Network Gas Token", "R5", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.r5.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.r5.network");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/337");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.R5, r5);
		
		// altblockscan
		EVMChainInfo altblockscan = new EVMChainInfo(
				EVMChain.ALTBLOCKSCAN.toString(), 
				"Altblockscan Mainnet", 
				191919L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Altblockscan Mainnet Gas Token", "ALTB", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.altblockscan.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.altblockscan.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/191919");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ALTBLOCKSCAN, altblockscan);
		
		// swisstronik
		EVMChainInfo swisstronik = new EVMChainInfo(
				EVMChain.SWISSTRONIK.toString(), 
				"Swisstronik Mainnet", 
				1848L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Swisstronik Mainnet Gas Token", "SWTR", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://json-rpc.mainnet.swisstronik.com/unencrypted/");
					this.add("https://json-rpc.mainnet.swisstronik.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer-cosmos.mainnet.swisstronik.com/swisstronik");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1848");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.SWISSTRONIK, swisstronik);
		
		// opengpu
		EVMChainInfo opengpu = new EVMChainInfo(
				EVMChain.OPENGPU.toString(), 
				"OpenGPU Mainnet", 
				1071L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("OpenGPU Mainnet Gas Token", "OGPU", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet-rpc.ogpuscan.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://ogpuscan.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1071");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.OPENGPU, opengpu);
		
		// choochain
		EVMChainInfo choochain = new EVMChainInfo(
				EVMChain.CHOOCHAIN.toString(), 
				"ChooChain", 
				247L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("ChooChain Gas Token", "CHOO", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.choochain.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://blocks.choochain.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/247");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.CHOOCHAIN, choochain);
		
		// shardeum
		EVMChainInfo shardeum = new EVMChainInfo(
				EVMChain.SHARDEUM.toString(), 
				"Shardeum", 
				8118L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Shardeum Gas Token", "SHM", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://api.shardeum.org/");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.shardeum.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/8118");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.SHARDEUM, shardeum);
		
		// firachain
		EVMChainInfo firachain = new EVMChainInfo(
				EVMChain.FIRACHAIN.toString(), 
				"firachain", 
				194L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("firachain Gas Token", "FIR", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.firachain.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://block.firachain.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/194");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.FIRACHAIN, firachain);
		
		// juneo
		EVMChainInfo juneosgd = new EVMChainInfo(
				EVMChain.JUNEOSGD1.toString(), 
				"Juneo SGD1-Chain", 
				45012L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Juneo SGD1-Chain Gas Token", "SGD1", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.juneo-mainnet.network/ext/bc/SGD1/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://juneoscan.io/chain/7");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/45012");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.JUNEOSGD1, juneosgd);
		
		// juneo
		EVMChainInfo juneobch1 = new EVMChainInfo(
				EVMChain.JUNEOBCH1.toString(), 
				"Juneo BCH1-Chain", 
				45013L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Juneo BCH1-Chain Gas Token", "BCH1", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.juneo-mainnet.network/ext/bc/BCH1/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://juneoscan.io/chain/12");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/45013");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.JUNEOBCH1, juneobch1);
		
		// juneo
		EVMChainInfo juneolink1 = new EVMChainInfo(
				EVMChain.JUNEOLINK1.toString(), 
				"Juneo LINK1-Chain", 
				45014L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Juneo LINK1-Chain Gas Token", "LINK1", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.juneo-mainnet.network/ext/bc/LINK1/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://juneoscan.io/chain/13");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/45014");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.JUNEOLINK1, juneolink1);
		
		// eventum
		EVMChainInfo eventum = new EVMChainInfo(
				EVMChain.EVENTUM.toString(), 
				"Eventum Mainnet", 
				161803L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Eventum Mainnet Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet-rpc.evedex.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.evedex.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/161803");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.EVENTUM, eventum);
		
		// socotra
		EVMChainInfo socotra = new EVMChainInfo(
				EVMChain.SOCOTRA.toString(), 
				"Socotra JUNE-Chain", 
				101003L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Socotra JUNE-Chain Gas Token", "JUNE", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.socotra-testnet.network/ext/bc/JUNE/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://socotra.juneoscan.io/chain/2");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/101003");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.SOCOTRA, socotra);
		
		// kub
		EVMChainInfo kub = new EVMChainInfo(
				EVMChain.KUB.toString(), 
				"KUB Mainnet", 
				96L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("KUB Mainnet Gas Token", "KUB", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.bitkubchain.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://kubscan.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/96");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.KUB, kub);
		
		// winr
		EVMChainInfo winr = new EVMChainInfo(
				EVMChain.WINR.toString(), 
				"Winr Protocol Mainnet", 
				777777L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Winr Protocol Mainnet Gas Token", "WINR", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.winr.games");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.winr.games");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/777777");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.WINR, winr);
		
		// primea
		EVMChainInfo primea = new EVMChainInfo(
				EVMChain.PRIMEA.toString(), 
				"Primea Network Mainnet", 
				698369L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Primea Network Mainnet Gas Token", "GOLDPN", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.primeanetwork.com/rpc-https");
					this.add("http://rpc.primeanetwork.com/rpc-http");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/698369");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.PRIMEA, primea);
		
		// perennial
		EVMChainInfo perennial = new EVMChainInfo(
				EVMChain.PERENNIAL.toString(), 
				"Perennial", 
				1424L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Perennial Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.perennial.foundation");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.perennial.foundation");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1424");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.PERENNIAL, perennial);
		
		// basedai
		EVMChainInfo basedai = new EVMChainInfo(
				EVMChain.BASEDAI.toString(), 
				"BasedAI", 
				32323L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("BasedAI Gas Token", "BASED", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet.basedaibridge.com/rpc/");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.bf1337.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/32323");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.BASEDAI, basedai);
		
		// inertia
		EVMChainInfo inertia = new EVMChainInfo(
				EVMChain.INERTIA.toString(), 
				"Inertia Scan", 
				5433L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Inertia Scan Gas Token", "IRTA", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.inertiascan.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://inertiascan.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/5433");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.INERTIA, inertia);
		
		// rogue
		EVMChainInfo rogue = new EVMChainInfo(
				EVMChain.ROGUE.toString(), 
				"Rogue Chain", 
				560013L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Rogue Chain Gas Token", "ROGUE", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.roguechain.io/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://roguescan.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/560013");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ROGUE, rogue);
		
		// plume
		EVMChainInfo plume2 = new EVMChainInfo(
				EVMChain.PLUME98866.toString(), 
				"Plume Mainnet", 
				98866L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Plume Mainnet Gas Token", "PLUME", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://phoenix-rpc.plumenetwork.xyz");
					this.add("https://rpc.plume.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://phoenix-explorer.plumenetwork.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/98866");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.PLUME98866, plume2);
		
		// miracle
		EVMChainInfo miracle = new EVMChainInfo(
				EVMChain.MIRACLE.toString(), 
				"Miracle Chain", 
				92278L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Miracle Chain Gas Token", "MPT", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.miracleplay.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.miracleplay.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/92278");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.MIRACLE, miracle);
		
		// settlus
		EVMChainInfo settlus = new EVMChainInfo(
				EVMChain.SETTLUS.toString(), 
				"Settlus", 
				5371L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Settlus Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://settlus-mainnet.g.alchemy.com/public");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://mainnet.settlus.network");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/5371");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.SETTLUS, settlus);
		
		// gunz
		EVMChainInfo gunz = new EVMChainInfo(
				EVMChain.GUNZ.toString(), 
				"GUNZ", 
				43419L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("GUNZ Gas Token", "GUN", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.gunzchain.io/ext/bc/2M47TxWHGnhNtq6pM5zPXdATBtuqubxn5EPFgFmEawCQr9WFML/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://gunzscan.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/43419");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.GUNZ, gunz);
		
		// juneo
		EVMChainInfo juneo = new EVMChainInfo(
				EVMChain.JUNEOJUNE.toString(), 
				"Juneo JUNE-Chain", 
				45003L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Juneo JUNE-Chain Gas Token", "JUNE", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.juneo-mainnet.network/ext/bc/JUNE/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://juneoscan.io/chain/2");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/45003");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.JUNEOJUNE, juneo);
		
		// juneo
		EVMChainInfo juneodai1 = new EVMChainInfo(
				EVMChain.JUNEODAI1.toString(), 
				"Juneo DAI1-Chain", 
				45004L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Juneo DAI1-Chain Gas Token", "DAI1", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.juneo-mainnet.network/ext/bc/DAI1/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://juneoscan.io/chain/5");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/45004");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.JUNEODAI1, juneodai1);
		
		// juneo
		EVMChainInfo juneousdt = new EVMChainInfo(
				EVMChain.JUNEOUSDT.toString(), 
				"Juneo USDT1-Chain", 
				45005L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Juneo USDT1-Chain Gas Token", "USDT1", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.juneo-mainnet.network/ext/bc/USDT1/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://juneoscan.io/chain/3");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/45005");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.JUNEOUSDT, juneousdt);
		
		// juneo
		EVMChainInfo juneousd = new EVMChainInfo(
				EVMChain.JUNEOUSD.toString(), 
				"Juneo USD1-Chain", 
				45006L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Juneo USD1-Chain Gas Token", "USD1", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.juneo-mainnet.network/ext/bc/USD1/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://juneoscan.io/chain/4");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/45006");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.JUNEOUSD, juneousd);
		
		// juneo
		EVMChainInfo juneogld = new EVMChainInfo(
				EVMChain.JUNEOGLD1.toString(), 
				"Juneo GLD1-Chain", 
				45008L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Juneo GLD1-Chain Gas Token", "GLD1", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.juneo-mainnet.network/ext/bc/GLD1/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://juneoscan.io/chain/8");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/45008");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.JUNEOGLD1, juneogld);
		
		// juneo
		EVMChainInfo juneoltc = new EVMChainInfo(
				EVMChain.JUNEOLTC1.toString(), 
				"Juneo LTC1-Chain", 
				45009L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Juneo LTC1-Chain Gas Token", "LTC1", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.juneo-mainnet.network/ext/bc/LTC1/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://juneoscan.io/chain/11");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/45009");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.JUNEOLTC1, juneoltc);
		
		// juneo
		EVMChainInfo juneodoge1 = new EVMChainInfo(
				EVMChain.JUNEODOGE1.toString(), 
				"Juneo DOGE1-Chain", 
				45010L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Juneo DOGE1-Chain Gas Token", "DOGE1", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.juneo-mainnet.network/ext/bc/DOGE1/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://juneoscan.io/chain/10");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/45010");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.JUNEODOGE1, juneodoge1);
		
		// juneo
		EVMChainInfo juneoeur = new EVMChainInfo(
				EVMChain.JUNEOEUR1.toString(), 
				"Juneo EUR1-Chain", 
				45011L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Juneo EUR1-Chain Gas Token", "EUR1", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.juneo-mainnet.network/ext/bc/EUR1/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://juneoscan.io/chain/6");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/45011");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.JUNEOEUR1, juneoeur);
		
		// juneo
		EVMChainInfo juneobtc1 = new EVMChainInfo(
				EVMChain.JUNEOBTC1.toString(), 
				"Juneo mBTC1-Chain", 
				45007L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Juneo mBTC1-Chain Gas Token", "mBTC1", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.juneo-mainnet.network/ext/bc/mBTC1/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://juneoscan.io/chain/9");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/45007");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.JUNEOBTC1, juneobtc1);
		
		// load
		EVMChainInfo load = new EVMChainInfo(
				EVMChain.LOAD.toString(), 
				"Load Alphanet", 
				9496L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Load Alphanet Gas Token", "tLOAD", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://alphanet.load.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.load.network");
				}},
				new ArrayList<String>() {{
					this.add("https://load.network/faucet");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/9496");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.LOAD, load);
		
		// pentagon
		EVMChainInfo pentagon = new EVMChainInfo(
				EVMChain.PENTAGON.toString(), 
				"Pentagon Chain", 
				3344L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Pentagon Chain Gas Token", "PC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.pentagon.games");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.pentagon.games");
				}},
				new ArrayList<String>() {{
					this.add("https://bridge.pentagon.games");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/3344");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.PENTAGON, pentagon);
		
		// onino
		EVMChainInfo onino = new EVMChainInfo(
				EVMChain.ONINO.toString(), 
				"ONINO Mainnet", 
				1425L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("ONINO Mainnet Gas Token", "ONI", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.onino.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.onino.io");
				}},
				new ArrayList<String>() {{
					this.add("https://faucet.onino.io");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1425");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ONINO, onino);
		
		// statemesh
		EVMChainInfo statemesh = new EVMChainInfo(
				EVMChain.STATEMESH.toString(), 
				"StateMesh", 
				1134L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("StateMesh Gas Token", "MESH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.statemesh.net");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.statemesh.net");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1134");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.STATEMESH, statemesh);
		
		// lens
		EVMChainInfo lens = new EVMChainInfo(
				EVMChain.LENS.toString(), 
				"Lens", 
				232L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Lens Gas Token", "GHO", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.lens.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.lens.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/232");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.LENS, lens);
		
		// idn
		EVMChainInfo idn = new EVMChainInfo(
				EVMChain.IDN.toString(), 
				"IDN Mainnet", 
				215L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("IDN Mainnet Gas Token", "IDN", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://dataseed1.idn-rpc.com");
					this.add("https://dataseed2.idn-rpc.com");
					this.add("https://dataseed3.idn-rpc.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.idn-network.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/215");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.IDN, idn);
		
		// draw
		EVMChainInfo draw = new EVMChainInfo(
				EVMChain.DRAW.toString(), 
				"Draw Coin", 
				7788L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Draw Coin Gas Token", "DRW", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.drawchain.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://drawscan.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/7788");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.DRAW, draw);
		
		// hemi
		EVMChainInfo hemi = new EVMChainInfo(
				EVMChain.HEMI.toString(), 
				"Hemi Network", 
				43111L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Hemi Network Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.hemi.network/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.hemi.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/43111");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.HEMI, hemi);
		
		// z
		EVMChainInfo z = new EVMChainInfo(
				EVMChain.Z.toString(), 
				"Z Chain", 
				9369L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Z Chain Gas Token", "Z", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.zchain.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://zscan.live");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/9369");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.Z, z);
		
		// pione
		EVMChainInfo pionez = new EVMChainInfo(
				EVMChain.PIONEZERO.toString(), 
				"Pione Zero", 
				5080L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Pione Zero Gas Token", "PZO", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.zeroscan.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://zero.pionescan.com");
				}},
				new ArrayList<String>() {{
					this.add("https://dex.pionechain.com/testnet/faucet/");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/5080");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.PIONEZERO, pionez);
		
		// blackfort
		EVMChainInfo blackfort = new EVMChainInfo(
				EVMChain.BLACKFORT.toString(), 
				"BlackFort Exchange Network Deprecated", 
				4999L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("BlackFort Exchange Network Deprecated Gas Token", "BXNdpr", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet.blackfort.network/rpc");
					this.add("https://mainnet-1.blackfort.network/rpc");
					this.add("https://mainnet-2.blackfort.network/rpc");
					this.add("https://mainnet-3.blackfort.network/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.blackfort.network");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/4999");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.BLACKFORT, blackfort);
		
		// pepe
		EVMChainInfo pepe = new EVMChainInfo(
				EVMChain.PEPE97741.toString(), 
				"PEPE Unchained", 
				97741L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("PEPE Unchained Gas Token", "PEPU", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc-pepu-v2-mainnet-0.t.conduit.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://pepuscan.com/");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/97741");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.PEPE97741, pepe);
		
		// phala
		EVMChainInfo phala = new EVMChainInfo(
				EVMChain.PHALA.toString(), 
				"Phala Network", 
				2035L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Phala Network Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.phala.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/2035");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.PHALA, phala);
		
		// datagram
		EVMChainInfo datagram = new EVMChainInfo(
				EVMChain.DATAGRAM.toString(), 
				"Datagram", 
				968L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Datagram Gas Token", "DGRAM", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet.datagram.network/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.datagram.network");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/968");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.DATAGRAM, datagram);
		
		// ql1
		EVMChainInfo ql1 = new EVMChainInfo(
				EVMChain.QL1.toString(), 
				"QL1", 
				766L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("QL1 Gas Token", "QOM", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.qom.one");
					this.add("https://evm-rpc-ql1.foxxone.one");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.qom.one");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/766");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.QL1, ql1);
		
		// zkxpla
		EVMChainInfo zkxpla = new EVMChainInfo(
				EVMChain.ZKXPLA.toString(), 
				"zkXPLA Mainnet", 
				375L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("zkXPLA Mainnet Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.zkxpla.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.zkxpla.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/375");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ZKXPLA, zkxpla);
		
		// redmansion
		EVMChainInfo redmansion = new EVMChainInfo(
				EVMChain.REDMANSION.toString(), 
				"Redmansion Chain", 
				192L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Redmansion Chain Gas Token", "RMC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://redmansion.io/srpc/");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://redmansion.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/192");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.REDMANSION, redmansion);
		
		// hyperevm
		EVMChainInfo hyperevm = new EVMChainInfo(
				EVMChain.HYPEREVM.toString(), 
				"HyperEVM", 
				999L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("HyperEVM Gas Token", "HYPE", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.hyperliquid.xyz/evm");
					this.add("https://rpc.hypurrscan.io");
					this.add("https://hyperliquid-json-rpc.stakely.io");
					this.add("https://hyperliquid.drpc.org");
					this.add("https://rpc.hyperlend.finance");
					this.add("https://hyperliquid.api.onfinality.io/evm/public");
					this.add("https://hyperliquid.rpc.blxrbdn.com");
					this.add("https://rpc.countzero.xyz/evm");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://purrsec.com/");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/999");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.HYPEREVM, hyperevm);
		
		// pepe
		EVMChainInfo pepe1 = new EVMChainInfo(
				EVMChain.PEPE1.toString(), 
				"Pepe Unchained", 
				3409L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Pepe Unchained Gas Token", "PEPU", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc-pepe-unchained-gupg0lo9wf.t.conduit.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://pepuscan.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/3409");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.PEPE1, pepe1);

		
		// reactive
		EVMChainInfo reactive = new EVMChainInfo(
				EVMChain.REACTIVE.toString(), 
				"Reactive Mainnet", 
				1597L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Reactive Mainnet Gas Token", "REACT", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet-rpc.rnk.dev");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://reactscan.net");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1597");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.REACTIVE, reactive);
		
		// pundi
		EVMChainInfo pundi = new EVMChainInfo(
				EVMChain.PUNDI.toString(), 
				"Pundi AIFX Omnilayer", 
				530L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Pundi AIFX Omnilayer Gas Token", "PUNDAI", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://fx-json-web3.functionx.io:8545");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://pundiscan.io/evm");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/530");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.PUNDI, pundi);
		
		// magichain
		EVMChainInfo magichain = new EVMChainInfo(
				EVMChain.MAGICHAIN.toString(), 
				"MagiChain", 
				24125L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("MagiChain Gas Token", "MGIX", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.magichain.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.magichain.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/24125");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.MAGICHAIN, magichain);

		
		// apertum
		EVMChainInfo apertum = new EVMChainInfo(
				EVMChain.APERTUM.toString(), 
				"Apertum", 
				2786L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Apertum Gas Token", "APTM", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.apertum.io/ext/bc/YDJ1r9RMkewATmA7B35q1bdV18aywzmdiXwd9zGBq3uQjsCnn/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.apertum.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/2786");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.APERTUM, apertum);
		
		// lagom
		EVMChainInfo lagom = new EVMChainInfo(
				EVMChain.LAGOM.toString(), 
				"LAGOM Mainnet", 
				986L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("LAGOM Mainnet Gas Token", "LAGO", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc1.lagom.mainnet.zeeve.net");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://Explorer.lagomchain.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/986");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.LAGOM, lagom);
		
		// coinweb
		EVMChainInfo coinweb = new EVMChainInfo(
				EVMChain.COINWEB.toString(), 
				"Coinweb BNB shard", 
				2222222L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Coinweb BNB shard Gas Token", "CWEB", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://api-cloud.coinweb.io/eth-rpc-service/bnb");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.coinweb.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/2222222");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.COINWEB, coinweb);
		
		// ethpar
		EVMChainInfo ethpar = new EVMChainInfo(
				EVMChain.ETHPAR.toString(), 
				"Ethpar Mainnet", 
				1727L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Ethpar Mainnet Gas Token", "ETP", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc01.ethpar.net/");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://dora.ethpar.net");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1727");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ETHPAR, ethpar);
		
		// smart
		EVMChainInfo smart = new EVMChainInfo(
				EVMChain.SMART.toString(), 
				"Smart Pay Mobile Money", 
				666301179999L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Smart Pay Mobile Money Gas Token", "SMRTP", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://network.uat.smartmoneyewallet.com/");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/666301179999");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.SMART, smart);
		
		// atleta
		EVMChainInfo atleta = new EVMChainInfo(
				EVMChain.ATLETA.toString(), 
				"Atleta Network", 
				2440L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Atleta Network Gas Token", "ATLA", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.mainnet.atleta.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://polkadot-explorer.atleta.network/#/explorer");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/2440");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ATLETA, atleta);

		// xferchain
		EVMChainInfo xferchain = new EVMChainInfo(
				EVMChain.XFERCHAIN.toString(), 
				"XferChain Mainnet", 
				28125L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("XferChain Mainnet Gas Token", "Dapo", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.xferchain.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://xferchain.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/28125");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.XFERCHAIN, xferchain);
		
		// tuxappcoin
		EVMChainInfo tuxappcoin = new EVMChainInfo(
				EVMChain.TUXAPPCOIN.toString(), 
				"Tuxappcoin", 
				1313161573L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Tuxappcoin Gas Token", "TUXA", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc-0x4e454165.aurora-cloud.dev");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.tuxa.aurora.dev");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1313161573");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.TUXAPPCOIN, tuxappcoin);
		
		// reactive
		EVMChainInfo reactivekopli = new EVMChainInfo(
				EVMChain.REACTIVEKOPLI.toString(), 
				"Reactive Kopli", 
				5318008L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Reactive Kopli Gas Token", "REACT", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://kopli-rpc.rnk.dev");
					this.add("http://kopli-rpc.rkt.ink");
					this.add("https://kopli-rpc.reactive.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://kopli.reactscan.net");
				}},
				new ArrayList<String>() {{
					this.add("https://dev.reactive.network/docs/kopli-testnet#faucet");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/5318008");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.REACTIVEKOPLI, reactivekopli);
		
		// emeraldz
		EVMChainInfo emeraldz = new EVMChainInfo(
				EVMChain.EMERALDZ.toString(), 
				"Emeraldz", 
				789789L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Emeraldz Gas Token", "EMED", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://public.0xrpc.com/789789");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://emeraldzscan.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/789789");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.EMERALDZ, emeraldz);
		
		// unichain
		EVMChainInfo unichain = new EVMChainInfo(
				EVMChain.UNICHAIN.toString(), 
				"Unichain", 
				130L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Unichain Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet.unichain.org");
					this.add("https://unichain-rpc.publicnode.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://unichain.blockscout.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/130");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.UNICHAIN, unichain);
		
		// nibiru_test
		EVMChainInfo nibiru_test = new EVMChainInfo(
				EVMChain.NIBIRUTEST.toString(), 
				"Nibiru testnet-1", 
				7210L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Nibiru testnet-1 Gas Token", "NIBI", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://evm-rpc.testnet-1.nibiru.fi");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/7210");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.NIBIRUTEST, nibiru_test);
		
		// memecore
		EVMChainInfo memecore = new EVMChainInfo(
				EVMChain.MEMECORE.toString(), 
				"MemeCore", 
				4352L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("MemeCore Gas Token", "M", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.memecore.net");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://blockscout.memecore.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/4352");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.MEMECORE, memecore);
		
		// sagaevm
		EVMChainInfo sagaevm = new EVMChainInfo(
				EVMChain.SAGAEVM.toString(), 
				"SagaEVM", 
				5464L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("SagaEVM Gas Token", "GAS", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://sagaevm.jsonrpc.sagarpc.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://sagaevm.sagaexplorer.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/5464");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.SAGAEVM, sagaevm);
		
		// pruv
		EVMChainInfo pruv = new EVMChainInfo(
				EVMChain.PRUV.toString(), 
				"Pruv Mainnet", 
				7337L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Pruv Mainnet Gas Token", "PRUV", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.pruv.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.pruv.network");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/7337");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.PRUV, pruv);
		
		// jamton
		EVMChainInfo jamton = new EVMChainInfo(
				EVMChain.JAMTON.toString(), 
				"Jamton", 
				5589L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Jamton Gas Token", "DOTON", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.jamton.network/");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/5589");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.JAMTON, jamton);
		
		// xone
		EVMChainInfo xone = new EVMChainInfo(
				EVMChain.XONE.toString(), 
				"Xone Mainnet", 
				3721L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Xone Mainnet Gas Token", "XOC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.xone.org");
					this.add("https://rpc-node-1.xone.org");
					this.add("https://rpc-node-2.xone.org");
					this.add("https://rpc-node-3.xone.org");
					this.add("https://rpc-node-4.xone.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://xscscan.com");
				}},
				new ArrayList<String>() {{
					this.add("https://faucet.xone.org/");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/3721");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.XONE, xone);
		
		// coinzax
		EVMChainInfo coinzax = new EVMChainInfo(
				EVMChain.COINZAX.toString(), 
				"COINZAX", 
				1310L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("COINZAX Gas Token", "ZAX", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.coinzax.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.coinzax.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1310");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.COINZAX, coinzax);
		
		// weber
		EVMChainInfo weber = new EVMChainInfo(
				EVMChain.WEBER.toString(), 
				"Weber Governance Mainnet", 
				881L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Weber Governance Mainnet Gas Token", "PTT", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://chain.myweber.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://mainnet.myweber.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/881");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.WEBER, weber);
		
		// landstars
		EVMChainInfo landstars = new EVMChainInfo(
				EVMChain.LANDSTARS.toString(), 
				"Landstars", 
				495L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Landstars Gas Token", "LDS", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://13882-60301.pph-server.de");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/495");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.LANDSTARS, landstars);
		
		// official
		EVMChainInfo official_vasyl = new EVMChainInfo(
				EVMChain.VASYL.toString(), 
				"OFFICIAL VASYL", 
				1278060416L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("OFFICIAL VASYL Gas Token", "VASYL", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.official-vasyl.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.official-vasyl.network");
				}},
				new ArrayList<String>() {{
					this.add("https://faucet.official-vasyl.network");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1278060416");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.VASYL, official_vasyl);
		

		// plume
		EVMChainInfo plumel = new EVMChainInfo(
				EVMChain.PLUMELEGACY.toString(), 
				"Plume (Legacy)", 
				98865L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Plume (Legacy) Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.plumenetwork.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.plumenetwork.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/98865");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.PLUMELEGACY, plumel);
		
		// xphere
		EVMChainInfo xphere = new EVMChainInfo(
				EVMChain.XPHERE.toString(), 
				"Xphere Mainnet", 
				20250217L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Xphere Mainnet Gas Token", "XP", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://en-bkk.x-phere.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/20250217");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.XPHERE, xphere);
		
		// plume
		EVMChainInfo plume = new EVMChainInfo(
				EVMChain.PLUME98865.toString(), 
				"Plume Mainnet", 
				98865L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Plume Mainnet Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.plumenetwork.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.plumenetwork.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/98865");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.PLUME98865, plume);
		
		// worldecomoney
		EVMChainInfo worldecomoney = new EVMChainInfo(
				EVMChain.WORLDECOMONEY.toString(), 
				"WorldEcoMoney", 
				315L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("WorldEcoMoney Gas Token", "WEM", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.wemblockchain.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://wemscan.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/315");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.WORLDECOMONEY, worldecomoney);
		
		// etp
		EVMChainInfo etp = new EVMChainInfo(
				EVMChain.ETP.toString(), 
				"ETP Mainnet", 
				20256789L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("ETP Mainnet Gas Token", "ETP", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.etpscan.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://etpscan.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/20256789");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ETP, etp);
		
		// deepbrainchain
		EVMChainInfo deepbrainchain = new EVMChainInfo(
				EVMChain.DEEPBRAINCHAIN.toString(), 
				"DeepBrainChain Mainnet", 
				19880818L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("DeepBrainChain Mainnet Gas Token", "DBC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.dbcwallet.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://www.dbcscan.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/19880818");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.DEEPBRAINCHAIN, deepbrainchain);
		
		// ternoa
		EVMChainInfo ternoa = new EVMChainInfo(
				EVMChain.TERNOA.toString(), 
				"Ternoa", 
				752025L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Ternoa Gas Token", "CAPS", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc-mainnet.zkevm.ternoa.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer-mainnet.zkevm.ternoa.network");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/752025");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.TERNOA, ternoa);
		
		// zether
		EVMChainInfo zether = new EVMChainInfo(
				EVMChain.ZETHER.toString(), 
				"Zether Mainnet", 
				715131L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Zether Mainnet Gas Token", "ZTH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.zether.org");
					this.add("https://rpc.zthscan.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://zthscan.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/715131");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ZETHER, zether);
		
		// fidesinnova
		EVMChainInfo fidesinnova = new EVMChainInfo(
				EVMChain.FIDESINNOVA.toString(), 
				"Fidesinnova", 
				706883L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Fidesinnova Gas Token", "FDS", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://fidesf1-rpc.fidesinnova.io");
					this.add("https://rpc1.fidesinnova.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.fidesinnova.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/706883");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.FIDESINNOVA, fidesinnova);

		
		// conwai
		EVMChainInfo conwai = new EVMChainInfo(
				EVMChain.CONWAI.toString(), 
				"Conwai Mainnet", 
				668668L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Conwai Mainnet Gas Token", "CNW", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://conwai.calderachain.xyz/http");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://conwai.calderaexplorer.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/668668");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.CONWAI, conwai);
		
		// graphite
		EVMChainInfo graphite = new EVMChainInfo(
				EVMChain.GRAPHITE.toString(), 
				"Graphite Mainnet", 
				440017L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Graphite Mainnet Gas Token", "@G", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://anon-entrypoint-1.atgraphite.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://main.atgraphite.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/440017");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.GRAPHITE, graphite);
		
		// infinaeon
		EVMChainInfo infinaeon = new EVMChainInfo(
				EVMChain.INFINAEON.toString(), 
				"Infinaeon", 
				420000L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Infinaeon Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.infinaeon.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.infinaeon.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/420000");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.INFINAEON, infinaeon);
		
		// conet
		EVMChainInfo conet = new EVMChainInfo(
				EVMChain.CONET.toString(), 
				"CONET Mainnet", 
				224400L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("CONET Mainnet Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet-rpc.conet.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://mainnet.conet.network");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/224400");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.CONET, conet);
		
		// ethereumfair
		EVMChainInfo ethereumfair = new EVMChainInfo(
				EVMChain.ETHEREUMFAIR.toString(), 
				"EthereumFair", 
				513100L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("EthereumFair Gas Token", "ETHF", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.etherfair.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://www.oklink.com/ethf");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/513100");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ETHEREUMFAIR, ethereumfair);
		
		// yominet
		EVMChainInfo yominet = new EVMChainInfo(
				EVMChain.YOMINET.toString(), 
				"Yominet", 
				428962654539583L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Yominet Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://jsonrpc-yominet-1.anvil.asia-southeast.initia.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/428962654539583");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.YOMINET, yominet);

		
		// tron
		EVMChainInfo tronn = new EVMChainInfo(
				EVMChain.TRONNILE.toString(), 
				"Tron Nile", 
				3448148188L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Tron Nile Gas Token", "TRX", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://nile.trongrid.io/jsonrpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://nile.tronscan.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/3448148188");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.TRONNILE, tronn);
		
		// tron
		EVMChainInfo trons = new EVMChainInfo(
				EVMChain.TRONSHASTA.toString(), 
				"Tron Shasta", 
				2494104990L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Tron Shasta Gas Token", "TRX", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://api.shasta.trongrid.io/jsonrpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://shasta.tronscan.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/2494104990");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.TRONSHASTA, trons);
		
		// vcitychain
		EVMChainInfo vcitychain = new EVMChainInfo(
				EVMChain.VCITYCHAIN.toString(), 
				"Vcitychain Mainnet", 
				20250825L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Vcitychain Mainnet Gas Token", "VCITY", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet-rpc.vcity.app");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://blockchain.vcity.app");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/20250825");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.VCITYCHAIN, vcitychain);
		
		// coti
		EVMChainInfo coti = new EVMChainInfo(
				EVMChain.COTI.toString(), 
				"COTI", 
				2632500L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("COTI Gas Token", "COTI", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet.coti.io/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://mainnet.cotiscan.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/2632500");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.COTI, coti);
		
		// hydration
		EVMChainInfo hydration = new EVMChainInfo(
				EVMChain.HYDRATION.toString(), 
				"Hydration", 
				222222L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Hydration Gas Token", "WETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.hydradx.cloud");
					this.add("https://hydration-rpc.n.dwellir.com");
					this.add("https://rpc.helikon.io/hydradx");
					this.add("https://hydration.dotters.network");
					this.add("https://hydration.ibp.network");
					this.add("https://rpc.cay.hydration.cloud");
					this.add("https://rpc.parm.hydration.cloud");
					this.add("https://rpc.roach.hydration.cloud");
					this.add("https://rpc.zipp.hydration.cloud");
					this.add("https://rpc.sin.hydration.cloud");
					this.add("https://rpc.coke.hydration.cloud");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.evm.hydration.cloud");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/222222");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.HYDRATION, hydration);
		
		// cratd2c
		EVMChainInfo cratd2c = new EVMChainInfo(
				EVMChain.CRATD2C.toString(), 
				"CratD2C", 
				2310L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("CratD2C Gas Token", "CRAT", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://node1.cratd2csmartchain.io");
					this.add("https://node2.cratd2csmartchain.io");
					this.add("https://node3.cratd2csmartchain.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.cratd2csmartchain.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/2310");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.CRATD2C, cratd2c);
		
		// rufus
		EVMChainInfo rufus = new EVMChainInfo(
				EVMChain.RUFUS.toString(), 
				"Rufus", 
				2420L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Rufus Gas Token", "ELON", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rufus.calderachain.xyz/http");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://rufus.calderaexplorer.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/2420");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.RUFUS, rufus);
		
		// hydra
		EVMChainInfo hydra = new EVMChainInfo(
				EVMChain.HYDRA.toString(), 
				"Hydra Chain", 
				4488L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Hydra Chain Gas Token", "HYDRA", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc-mainnet.hydrachain.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://skynet.hydrachain.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/4488");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.HYDRA, hydra);
		
		// anychain
		EVMChainInfo anychain = new EVMChainInfo(
				EVMChain.ANYCHAIN.toString(), 
				"AnyChain", 
				5108L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("AnyChain Gas Token", "ANC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet.anychainscan.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://anychainscan.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/5108");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ANYCHAIN, anychain);
		
		// msc
		EVMChainInfo msc = new EVMChainInfo(
				EVMChain.MSC.toString(), 
				"MSC Blockchain", 
				5577L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("MSC Blockchain Gas Token", "MSC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.taaqo.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/5577");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.MSC, msc);
		
		// tburn
		EVMChainInfo tburn = new EVMChainInfo(
				EVMChain.TBURN.toString(), 
				"TBURN Mainnet", 
				5800L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("TBURN Mainnet Gas Token", "TBURN", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://tburn.io/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://tburn.io/scan");
				}},
				new ArrayList<String>() {{
					this.add("https://tburn.io/faucet");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/5800");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.TBURN, tburn);
		
		// bot
		EVMChainInfo bot = new EVMChainInfo(
				EVMChain.BOT.toString(), 
				"BOT Chain Mainnet", 
				677L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("BOT Chain Mainnet Gas Token", "BOT", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.botchain.ai");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.botchain.ai/");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/677");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.BOT, bot);
		
		// gelatine
		EVMChainInfo gelatine = new EVMChainInfo(
				EVMChain.GELATINE.toString(), 
				"Gelatine Network", 
				61564L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Gelatine Network Gas Token", "JELLO", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.pine.ink");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.pine.ink");
				}},
				new ArrayList<String>() {{
					this.add("https://gelatine.pine.ink");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/61564");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.GELATINE, gelatine);
		
		// somnia
		EVMChainInfo somnia = new EVMChainInfo(
				EVMChain.SOMNIA.toString(), 
				"Somnia Mainnet", 
				5031L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Somnia Mainnet Gas Token", "SOMI", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://api.infra.mainnet.somnia.network");
					this.add("https://somnia-json-rpc.stakely.io");
					this.add("https://somnia-rpc.publicnode.com");
					this.add("https://rpc.ankr.com/somnia_mainnet");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.somnia.network");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/5031");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.SOMNIA, somnia);
		
		// ethereal
		EVMChainInfo ethereal = new EVMChainInfo(
				EVMChain.ETHEREAL.toString(), 
				"Ethereal Mainnet", 
				5064014L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Ethereal Mainnet Gas Token", "USDe", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.ethereal.trade");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.ethereal.trade");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/5064014");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ETHEREAL, ethereal);
		
		// camp
		EVMChainInfo camp = new EVMChainInfo(
				EVMChain.CAMP.toString(), 
				"Camp Network Mainnet", 
				484L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Camp Network Mainnet Gas Token", "CAMP", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.camp.raas.gelato.cloud");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://camp.cloud.blockscout.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/484");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.CAMP, camp);
		
		// mintraxchain
		EVMChainInfo mintraxchain = new EVMChainInfo(
				EVMChain.MINTRAXCHAIN.toString(), 
				"MintraxChain", 
				478549L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("MintraxChain Gas Token", "MTX", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.mintrax.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.mintrax.network");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/478549");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.MINTRAXCHAIN, mintraxchain);
		
		// unipoly
		EVMChainInfo unipoly = new EVMChainInfo(
				EVMChain.UNIPOLY.toString(), 
				"Unipoly Chain Mainnet", 
				47382916L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Unipoly Chain Mainnet Gas Token", "UNP", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.unpchain.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.unpchain.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/47382916");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.UNIPOLY, unipoly);
		
		// areum
		EVMChainInfo areum = new EVMChainInfo(
				EVMChain.AREUM.toString(), 
				"Areum Mainnet", 
				463L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Areum Mainnet Gas Token", "AREA", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet-rpc.areum.network");
					this.add("https://mainnet-rpc2.areum.network");
					this.add("https://mainnet-rpc3.areum.network");
					this.add("https://mainnet-rpc4.areum.network");
					this.add("https://mainnet-rpc5.areum.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.areum.network");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/463");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.AREUM, areum);
		
		// mst
		EVMChainInfo mst = new EVMChainInfo(
				EVMChain.MST.toString(), 
				"MST Chain", 
				4646L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("MST Chain Gas Token", "MST", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mariorpc.mstblockchain.com");
					this.add("https://craftrpc.mstblockchain.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://mstscan.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/4646");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.MST, mst);
		
		// oev
		EVMChainInfo oev = new EVMChainInfo(
				EVMChain.OEV.toString(), 
				"OEV Network", 
				4913L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("OEV Network Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://oev.rpc.api3.org");
					this.add("https://oev-network.calderachain.xyz/http");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://oev-network.calderaexplorer.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/4913");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.OEV, oev);
		
		// saga
		EVMChainInfo saga = new EVMChainInfo(
				EVMChain.SAGA.toString(), 
				"Saga", 
				5464L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Saga Gas Token", "GAS", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("http://sagaevm-5464-1.jsonrpc.sagarpc.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://sagaevm-5464-1.sagaexplorer.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/5464");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.SAGA, saga);
		
		// sidra
		EVMChainInfo sidra = new EVMChainInfo(
				EVMChain.SIDRA.toString(), 
				"Sidra Chain", 
				97453L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Sidra Chain Gas Token", "SDA", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://node.sidrachain.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://ledger.sidrachain.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/97453");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.SIDRA, sidra);
		
		// henez
		EVMChainInfo henez = new EVMChainInfo(
				EVMChain.HENEZ.toString(), 
				"Henez Chain Mainnet", 
				91111L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Henez Chain Mainnet Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://henez.calderachain.xyz/http");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://henez.calderaexplorer.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/91111");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.HENEZ, henez);
		
		// o
		EVMChainInfo o = new EVMChainInfo(
				EVMChain.O.toString(), 
				"O Chain", 
				84841L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("O Chain Gas Token", "O", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.o.xyz");
					this.add("https://84841.rpc.thirdweb.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.o.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/84841");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.O, o);
		
		// vemp
		EVMChainInfo vemp = new EVMChainInfo(
				EVMChain.VEMP.toString(), 
				"VEMP Horizon", 
				82614L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("VEMP Horizon Gas Token", "VEMP", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://vemp-horizon.calderachain.xyz/http");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://vemp-horizon.calderaexplorer.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/82614");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.VEMP, vemp);
		
		// forta
		EVMChainInfo forta = new EVMChainInfo(
				EVMChain.FORTA.toString(), 
				"Forta Chain", 
				80931L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Forta Chain Gas Token", "FORT", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc-forta-chain-8gj1qndmfc.t.conduit.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.forta.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/80931");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.FORTA, forta);
		
		// geo
		EVMChainInfo geo = new EVMChainInfo(
				EVMChain.GEO.toString(), 
				"Geo Genesis", 
				80451L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Geo Genesis Gas Token", "GRT", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc-geo-genesis-h0q2s21xx8.t.conduit.xyz/");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/80451");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.GEO, geo);
		
		// caga
		EVMChainInfo caga = new EVMChainInfo(
				EVMChain.CAGA.toString(), 
				"CAGA mainnet", 
				72888L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("CAGA mainnet Gas Token", "CAGA", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://cagamainnet.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.cagamainnet.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/72888");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.CAGA, caga);
		
		// cyberchain
		EVMChainInfo cyberchain = new EVMChainInfo(
				EVMChain.CYBERCHAIN.toString(), 
				"CyberChain Mainnet", 
				65535L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("CyberChain Mainnet Gas Token", "CCX", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.cyberchain.xyz/");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.cyberchain.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/65535");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.CYBERCHAIN, cyberchain);
		
		// treasure
		EVMChainInfo treasure = new EVMChainInfo(
				EVMChain.TREASURE.toString(), 
				"Treasure", 
				61166L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Treasure Gas Token", "MAGIC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.treasure.lol");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://treasurescan.io");
				}},
				new ArrayList<String>() {{
					this.add("https://thirdweb.com/treasure");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/61166");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.TREASURE, treasure);

		
		// ink
		EVMChainInfo ink = new EVMChainInfo(
				EVMChain.INK.toString(), 
				"Ink", 
				57073L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Ink Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc-gel.inkonchain.com");
					this.add("https://rpc-qnd.inkonchain.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.inkonchain.com");
				}},
				new ArrayList<String>() {{
					this.add("https://inkonchain.com/faucet");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/57073");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.INK, ink);
		
		// overprotocol
		EVMChainInfo overprotocol = new EVMChainInfo(
				EVMChain.OVERPROTOCOL.toString(), 
				"OverProtocol Mainnet", 
				54176L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("OverProtocol Mainnet Gas Token", "OVER", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.overprotocol.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.over.network");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/54176");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.OVERPROTOCOL, overprotocol);
		
		// donatuz
		EVMChainInfo donatuz = new EVMChainInfo(
				EVMChain.DONATUZ.toString(), 
				"Donatuz", 
				42026L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Donatuz Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.donatuz.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.donatuz.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/42026");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.DONATUZ, donatuz);
		
		// pmon
		EVMChainInfo pmon = new EVMChainInfo(
				EVMChain.PMON.toString(), 
				"PMON Chain", 
				42001L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("PMON Chain Gas Token", "PMON", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.pmon.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.pmon.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/42001");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.PMON, pmon);
		
		// ferrum
		EVMChainInfo ferrum = new EVMChainInfo(
				EVMChain.FERRUM.toString(), 
				"Ferrum Quantum Portal Network", 
				26100L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Ferrum Quantum Portal Network Gas Token", "qpFRM", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://qpn.svcs.ferrumnetwork.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.ferrumnetwork.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/26100");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.FERRUM, ferrum);
		
		// ronin
		EVMChainInfo ronin = new EVMChainInfo(
				EVMChain.RONIN.toString(), 
				"Ronin Mainnet", 
				2020L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Ronin Mainnet Gas Token", "RON", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://api.roninchain.com/rpc");
					this.add("https://ronin.lgns.net/rpc");
					this.add("https://ronin.drpc.org");
					this.add("https://api-gateway.skymavis.com/rpc?apikey=9aqYLBbxSC6LROynQJBvKkEIsioqwHmr");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://app.roninchain.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/2020");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.RONIN, ronin);
		
		// premiumblock
		EVMChainInfo premiumblock = new EVMChainInfo(
				EVMChain.PREMIUMBLOCK.toString(), 
				"PremiumBlock", 
				23023L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("PremiumBlock Gas Token", "PBLK", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.premiumblock.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.premiumblock.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/23023");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.PREMIUMBLOCK, premiumblock);
		
		// ultra
		EVMChainInfo ultra = new EVMChainInfo(
				EVMChain.ULTRA.toString(), 
				"Ultra EVM Network", 
				19991L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Ultra EVM Network Gas Token", "UOS", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://evm.ultra.eosusa.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://evmexplorer.ultra.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/19991");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ULTRA, ultra);
		
		// abstract
		EVMChainInfo abstracto = new EVMChainInfo(
				EVMChain.ABSTRACT.toString(), 
				"Abstract", 
				2741L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Abstract Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://api.mainnet.abs.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.mainnet.abs.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/2741");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ABSTRACT, abstracto);
		
		// bitlazer
		EVMChainInfo bitlazer = new EVMChainInfo(
				EVMChain.BITLAZER.toString(), 
				"Bitlazer", 
				14235L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Bitlazer Gas Token", "lzrBTC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://bitlazer.calderachain.xyz/http");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://bitlazer.calderaexplorer.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/14235");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.BITLAZER, bitlazer);
		
		// huddle01
		EVMChainInfo huddle01 = new EVMChainInfo(
				EVMChain.HUDDLE01.toString(), 
				"Huddle01 dRTC Chain", 
				12323L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Huddle01 dRTC Chain Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://huddle01.calderachain.xyz/http");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://huddle01.calderaexplorer.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/12323");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.HUDDLE01, huddle01);
		
		// artela
		EVMChainInfo artela = new EVMChainInfo(
				EVMChain.ARTELA.toString(), 
				"Artela Mainnet", 
				11820L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Artela Mainnet Gas Token", "ART", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://node-euro.artela.network/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://artscan.artela.network");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/11820");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ARTELA, artela);
		
		// egold
		EVMChainInfo egold = new EVMChainInfo(
				EVMChain.EGOLD.toString(), 
				"eGold Chain", 
				11451L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("eGold Chain Gas Token", "XAU", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.egoldchain.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://egoldscan.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/11451");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.EGOLD, egold);
		
		// bitcoin
		EVMChainInfo bitcoin = new EVMChainInfo(
				EVMChain.BITCOIN.toString(), 
				"Bitcoin Chain", 
				8086L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Bitcoin Chain Gas Token", "BTC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.bitcoinevm.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/8086");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.BITCOIN, bitcoin);
		
		// powerloom2
		EVMChainInfo powerloom2 = new EVMChainInfo(
				EVMChain.POWERLOOM2.toString(), 
				"Powerloom Mainnet V2", 
				7869L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Powerloom Mainnet V2 Gas Token", "POWER", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc-v2.powerloom.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer-v2.powerloom.network");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/7869");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.POWERLOOM2, powerloom2);
		
		// gebs
		EVMChainInfo gebs = new EVMChainInfo(
				EVMChain.GEBSIGNET.toString(), 
				"GEB Signet", 
				11504L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("GEB Signet Gas Token", "BTC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://signet.geb.network/");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan-signet.geb.network");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/11504");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.GEBSIGNET, gebs);
		
		// nexurachain
		EVMChainInfo nexurachain = new EVMChainInfo(
				EVMChain.NEXURACHAIN.toString(), 
				"Nexurachain", 
				24125L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Nexurachain Gas Token", "XURA", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.nexurachain.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.nexurachain.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/24125");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.NEXURACHAIN, nexurachain);
		
		// esports
		EVMChainInfo esports = new EVMChainInfo(
				EVMChain.ESPORTS.toString(), 
				"Esports Chain", 
				17735L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Esports Chain Gas Token", "ESPT", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://esportsblock.org/rpc/");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://esportsblock.org/explorer");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/17735");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ESPORTS, esports);
		
		// geb
		EVMChainInfo geb = new EVMChainInfo(
				EVMChain.GEB.toString(), 
				"GEB Mainnet", 
				11501L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("GEB Mainnet Gas Token", "BTC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc-mainnet-2.geb.network/");
					this.add("https://rpc-mainnet-1.geb.network/");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://www.oklink.com/bevm");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/11501");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.GEB, geb);
		
		// powerloom
		EVMChainInfo powerloom = new EVMChainInfo(
				EVMChain.POWERLOOM.toString(), 
				"Powerloom Mainnet", 
				7865L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Powerloom Mainnet Gas Token", "POWER", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.powerloom.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.powerloom.network");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/7865");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.POWERLOOM, powerloom);
		
		
		
		// initverse
		EVMChainInfo initverse = new EVMChainInfo(
				EVMChain.INITVERSE.toString(), 
				"InitVerse mainnet", 
				7233L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("InitVerse mainnet Gas Token", "INI", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc-mainnet.inichain.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://www.iniscan.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/7233");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.INITVERSE, initverse);
		
		// haven1
		EVMChainInfo haven1 = new EVMChainInfo(
				EVMChain.HAVEN1.toString(), 
				"Haven1", 
				8811L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Haven1 Gas Token", "H1", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.haven1.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.haven1.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/8811");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.HAVEN1, haven1);
		
		// bharat
		EVMChainInfo bharat = new EVMChainInfo(
				EVMChain.BHARAT.toString(), 
				"Bharat Blockchain Network Mainnet", 
				8099L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Bharat Blockchain Network Mainnet Gas Token", "BBN", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://bbnrpc.mainnet.bharatblockchain.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/8099");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.BHARAT, bharat);
		
		// pointpay
		EVMChainInfo pointpay = new EVMChainInfo(
				EVMChain.POINTPAY.toString(), 
				"PointPay Mainnet", 
				5511L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("PointPay Mainnet Gas Token", "PXP", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc-mainnet.pointpay.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.pointpay.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/5511");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.POINTPAY, pointpay);
		
		// echos
		EVMChainInfo echos = new EVMChainInfo(
				EVMChain.ECHOS.toString(), 
				"Echos Chain", 
				4321L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Echos Chain Gas Token", "USDC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc-echos-mainnet-0.t.conduit.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.echos.fun");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/4321");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ECHOS, echos);
		
		// paynetwork
		EVMChainInfo paynetwork = new EVMChainInfo(
				EVMChain.PAYNETWORK.toString(), 
				"PayNetwork Mainnet", 
				3969L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("PayNetwork Mainnet Gas Token", "Pay", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.paynetwork.io");
					this.add("https://paynetwork-main.calderachain.xyz/http");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://paynetwork-main.calderaexplorer.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/3969");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.PAYNETWORK, paynetwork);
		
		// mezo
		EVMChainInfo mezo = new EVMChainInfo(
				EVMChain.MEZO.toString(), 
				"Mezo", 
				31612L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Mezo Gas Token", "BTC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://jsonrpc-mezo.boar.network");
					this.add("https://mainnet.mezo.public.validationcloud.io");
					this.add("https://rpc-internal.mezo.org");
					this.add("https://rpc_evm-mezo.imperator.co");
					this.add("https://rpc-http.mezo.boar.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.mezo.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/31612");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.MEZO, mezo);
		
		// firechain
		EVMChainInfo firechain = new EVMChainInfo(
				EVMChain.FIRECHAIN.toString(), 
				"Firechain zkEVM Ghostrider", 
				3885L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Firechain zkEVM Ghostrider Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc-zkevm-ghostrider.firestation.io");
					this.add("https://rpc-zkevm-ghostrider.thefirechain.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://ghostrider-zkevm.firescan.io");
				}},
				new ArrayList<String>() {{
					this.add("https://zkevm-faucet.firestation.io");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/3885");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.FIRECHAIN, firechain);

		
		// meroneum
		EVMChainInfo meroneum = new EVMChainInfo(
				EVMChain.MERONEUM.toString(), 
				"Meroneum", 
				3366L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Meroneum Gas Token", "MERON", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet-node1.meronscan.ai/");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://meronscan.ai");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/3366");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.MERONEUM, meroneum);
		
		// adi
		EVMChainInfo adi = new EVMChainInfo(
				EVMChain.ADI.toString(), 
				"ADI Chain", 
				36900L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("ADI Chain Gas Token", "ADI", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.adifoundation.ai");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.adifoundation.ai");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/36900");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ADI, adi);
		
		// billions
		EVMChainInfo billions = new EVMChainInfo(
				EVMChain.BILLIONS.toString(), 
				"Billions", 
				45056L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Billions Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://billions-rpc.eu-north-2.gateway.fm");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://billions-blockscout.eu-north-2.gateway.fm");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/45056");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.BILLIONS, billions);
		
		// nebula
		EVMChainInfo nebula = new EVMChainInfo(
				EVMChain.NEBULA.toString(), 
				"Nebula", 
				73285L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Nebula Gas Token", "NEBX", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://nebula-chain.com/rpc");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://nebxscan.nebula-chain.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/73285");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.NEBULA, nebula);
		
		// ethernova
		EVMChainInfo ethernova = new EVMChainInfo(
				EVMChain.ETHERNOVA.toString(), 
				"Ethernova Mainnet", 
				121525L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Ethernova Mainnet Gas Token", "NOVA", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.ethnova.net");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.ethnova.net");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/121525");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ETHERNOVA, ethernova);
		
		// hypercluster
		EVMChainInfo hypercluster = new EVMChainInfo(
				EVMChain.HYPERCLUSTER.toString(), 
				"HyperCluster", 
				838838L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("HyperCluster Gas Token", "HYPEC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.hypercluster.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.hypercluster.org");
				}},
				new ArrayList<String>() {{
					this.add("https://faucet.hypercluster.org");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/838838");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.HYPERCLUSTER, hypercluster);
		
		// xertra
		EVMChainInfo xertra = new EVMChainInfo(
				EVMChain.XERTRA.toString(), 
				"Xertra Mainnet", 
				105105L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Xertra Mainnet Gas Token", "STRAX", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.xertra.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.xertra.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/105105");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.XERTRA, xertra);
		
		// katron
		EVMChainInfo katron = new EVMChainInfo(
				EVMChain.KATRON.toString(), 
				"Katron AI Mainnet", 
				83592L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Katron AI Mainnet Gas Token", "KTN", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://blockchain-rpc1.katronai.com");
					this.add("https://blockchain-rpc2.katronai.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://ktnscan.katronai.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/83592");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.KATRON, katron);
		
		// bc
		EVMChainInfo bc = new EVMChainInfo(
				EVMChain.BC.toString(), 
				"BC Hyper Chain Mainnet", 
				3030L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("BC Hyper Chain Mainnet Gas Token", "VTCN", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainapi.bchscan.io");
					this.add("https://datahub-asia02.bchscan.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://mainnet.bchscan.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/3030");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.BC, bc);
		
		// goat
		EVMChainInfo goat = new EVMChainInfo(
				EVMChain.GOAT.toString(), 
				"GOAT Network", 
				2345L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("GOAT Network Gas Token", "BTC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.goat.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.goat.network");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/2345");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.GOAT, goat);
		
		// soneium
		EVMChainInfo soneium = new EVMChainInfo(
				EVMChain.SONEIUM.toString(), 
				"Soneium", 
				1868L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Soneium Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.soneium.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://soneium.slam.vision");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1868");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.SONEIUM, soneium);
		
		// glue
		EVMChainInfo glue = new EVMChainInfo(
				EVMChain.GLUE.toString(), 
				"Glue Mainnet", 
				1300L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Glue Mainnet Gas Token", "GLUE", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.glue.net");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.glue.net");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1300");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.GLUE, glue);
		
		// edu
		EVMChainInfo edu = new EVMChainInfo(
				EVMChain.EDU.toString(), 
				"EDU Chain", 
				41923L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("EDU Chain Gas Token", "EDU", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.edu-chain.raas.gelato.cloud");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://educhain.blockscout.com");
				}},
				new ArrayList<String>() {{
					this.add("https://bridge.gelato.network/bridge/edu-chain");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/41923");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.EDU, edu);
		
		// hashkey
		EVMChainInfo hashkey = new EVMChainInfo(
				EVMChain.HASHKEY.toString(), 
				"HashKey Chain", 
				177L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("HashKey Chain Gas Token", "HSK", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet.hsk.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.hsk.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/177");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.HASHKEY, hashkey);
		
		// wowchain
		EVMChainInfo wowchain = new EVMChainInfo(
				EVMChain.WOWCHAIN.toString(), 
				"WowChain Mainnet", 
				203L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("WowChain Mainnet Gas Token", "DOGE", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.wowchain.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.wowchain.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/203");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.WOWCHAIN, wowchain);
		
		// mind
		EVMChainInfo mind = new EVMChainInfo(
				EVMChain.MIND.toString(), 
				"Mind Network Mainnet", 
				228L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Mind Network Mainnet Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc-mainnet.mindnetwork.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/228");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.MIND, mind);
		
		// xr
		EVMChainInfo xr = new EVMChainInfo(
				EVMChain.XR.toString(), 
				"XR One", 
				273L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("XR One Gas Token", "XR", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://xr-one.calderachain.xyz/http");
					this.add("https://xr1.calderachain.xyz/http");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://xr-one.calderaexplorer.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/273");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.XR, xr);
		
		// zkcandy
		EVMChainInfo zkcandy = new EVMChainInfo(
				EVMChain.ZKCANDY.toString(), 
				"ZKcandy Mainnet", 
				320L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("ZKcandy Mainnet Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.zkcandy.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.zkcandy.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/320");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ZKCANDY, zkcandy);
		
		// amaxsmartchain
		EVMChainInfo amaxsmartchain = new EVMChainInfo(
				EVMChain.AMAXSMARTCHAIN.toString(), 
				"AmaxSmartchain", 
				662L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("AmaxSmartchain Gas Token", "AMAX", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.amaxchain.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.amaxchain.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/662");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.AMAXSMARTCHAIN, amaxsmartchain);
		
		// lydia
		EVMChainInfo lydia = new EVMChainInfo(
				EVMChain.LYDIA.toString(), 
				"Lydia Coin", 
				1989L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Lydia Coin Gas Token", "BSW", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.lydiacoins.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1989");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.LYDIA, lydia);
		
		// pho
		EVMChainInfo pho = new EVMChainInfo(
				EVMChain.PHO.toString(), 
				"Pho Blockchain Mainnet", 
				2605L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Pho Blockchain Mainnet Gas Token", "PHO", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet.phochain.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://phoscan.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/2605");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.PHO, pho);
		
		// witness
		EVMChainInfo witness = new EVMChainInfo(
				EVMChain.WITNESS.toString(), 
				"WITNESS CHAIN", 
				1702448187L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("WITNESS CHAIN Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://sequencer.witnesschain.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.witnesschain.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1702448187");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.WITNESS, witness);
		
		// privix
		EVMChainInfo privix = new EVMChainInfo(
				EVMChain.PRIVIX.toString(), 
				"Privix Chain Mainnet", 
				16969696L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Privix Chain Mainnet Gas Token", "PSC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet-rpc.privixchain.xyz/");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://privixscan.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/16969696");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.PRIVIX, privix);
		
		// conet
		EVMChainInfo conet_canun = new EVMChainInfo(
				EVMChain.CONETCANUN.toString(), 
				"CONET Cancun", 
				224433L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("CONET Cancun Gas Token", "CONET", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.conet.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://cancun.conet.network");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/224433");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.CONETCANUN, conet_canun);
		
		// lumoz
		EVMChainInfo lumoz = new EVMChainInfo(
				EVMChain.LUMOZ.toString(), 
				"Lumoz Chain Mainnet", 
				96370L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Lumoz Chain Mainnet Gas Token", "MOZ", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.lumoz.org");
					this.add("https://rpc-hk.lumoz.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.lumoz.info");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/96370");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.LUMOZ, lumoz);
		
		// arcadia
		EVMChainInfo arcadia = new EVMChainInfo(
				EVMChain.ARCADIA.toString(), 
				"Arcadia Mainnet", 
				4278608L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Arcadia Mainnet Gas Token", "AIP", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://arcadia.khalani.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.arcadia.khalani.network");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/4278608");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ARCADIA, arcadia);
		
		// oone
		EVMChainInfo oone = new EVMChainInfo(
				EVMChain.OONE.toString(), 
				"Oone Chain Mainnet", 
				777888L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Oone Chain Mainnet Gas Token", "OONE", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.oonechain.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://oonescan.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/777888");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.OONE, oone);
		
		// heima
		EVMChainInfo heima = new EVMChainInfo(
				EVMChain.HEIMA.toString(), 
				"Heima", 
				212013L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Heima Gas Token", "HEI", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.litentry-parachain.litentry.io");
					this.add("https://litentry-rpc.dwellir.com");
					this.add("https://rpc.heima-parachain.heima.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://litentry-rpc.dwellir.com");
					this.add("https://heima.statescan.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/212013");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.HEIMA, heima);
		
		// onfa
		EVMChainInfo onfa = new EVMChainInfo(
				EVMChain.ONFA.toString(), 
				"ONFA Chain", 
				8691942025L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("ONFA Chain Gas Token", "OFC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.onfa.io");
					this.add("https://rpc.onfachain.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://onfascan.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/8691942025");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ONFA, onfa);
		
		// redbelly
		EVMChainInfo redbelly = new EVMChainInfo(
				EVMChain.REDBELLY.toString(), 
				"Redbelly Network Mainnet", 
				151L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Redbelly Network Mainnet Gas Token", "RBNT", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://governors.mainnet.redbelly.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://redbelly.routescan.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/151");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.REDBELLY, redbelly);
		
		// mud
		EVMChainInfo mud = new EVMChainInfo(
				EVMChain.MUD.toString(), 
				"MUD Chain", 
				168169L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("MUD Chain Gas Token", "MUD", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.mud.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.mud.network");
				}},
				new ArrayList<String>() {{
					this.add("https://faucet.mud.network");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/168169");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.MUD, mud);
		
		// berachain
		EVMChainInfo berachain = new EVMChainInfo(
				EVMChain.BERACHAIN.toString(), 
				"Berachain", 
				80094L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Berachain Gas Token", "BERA", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.berachain.com");
					this.add("https://berachain-rpc.publicnode.com");
					this.add("https://rpc.berachain-apis.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://beratrail.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/80094");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.BERACHAIN, berachain);
		
		// birdlayer
		EVMChainInfo birdlayer = new EVMChainInfo(
				EVMChain.BIRDLAYER.toString(), 
				"BirdLayer", 
				53456L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("BirdLayer Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc1.birdlayer.xyz");
					this.add("https://rpc.birdlayer.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.birdlayer.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/53456");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.BIRDLAYER, birdlayer);
		
		// bevm
		EVMChainInfo bevm = new EVMChainInfo(
				EVMChain.BEVMSIGNET.toString(), 
				"BEVM Signet", 
				11504L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("BEVM Signet Gas Token", "BTC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://signet.bevm.io/");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan-signet.bevm.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/11504");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.BEVMSIGNET, bevm);
		
		// volcano
		EVMChainInfo volcano = new EVMChainInfo(
				EVMChain.VOLCANO.toString(), 
				"Volcano Chain Mainnet", 
				10085L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Volcano Chain Mainnet Gas Token", "VC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet.vchain.pro");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://vchainscan.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/10085");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.VOLCANO, volcano);
		
		// nibiru
		EVMChainInfo nibiru = new EVMChainInfo(
				EVMChain.NIBIRU.toString(), 
				"Nibiru Mainnet", 
				6900L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Nibiru Mainnet Gas Token", "NIBI", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://evm-rpc.nibiru.fi");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/6900");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.NIBIRU, nibiru);
		
		// pione
		EVMChainInfo pione = new EVMChainInfo(
				EVMChain.PIONE.toString(), 
				"Pione Chain Mainnet", 
				5090L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Pione Chain Mainnet Gas Token", "PIO", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.pionescan.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://pionescan.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/5090");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.PIONE, pione);
		
		// river
		EVMChainInfo river = new EVMChainInfo(
				EVMChain.RIVER.toString(), 
				"River", 
				550L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("River Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://towns-mainnet.calderachain.xyz/http");
					this.add("https://mainnet.rpc.river.build");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://towns-mainnet.calderaexplorer.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/550");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.RIVER, river);
		
		// nfb
		EVMChainInfo nfb = new EVMChainInfo(
				EVMChain.NFB.toString(), 
				"NFB Chain", 
				632L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("NFB Chain Gas Token", "NFBC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://node.nfbchain.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.nfbchain.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/632");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.NFB, nfb);
		
		// trex
		EVMChainInfo trex = new EVMChainInfo(
				EVMChain.TREX.toString(), 
				"T-Rex", 
				1628L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("T-Rex Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.trex.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.trex.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1628");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.TREX, trex);

		// skale
		EVMChainInfo skale = new EVMChainInfo(
				EVMChain.SKALE.toString(), 
				"SKALE Base", 
				1187947933L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("SKALE Base Gas Token", "CREDIT", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://skale-base.skalenodes.com/v1/base");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://skale-base-explorer.skalenodes.com");
				}},
				new ArrayList<String>() {{
					this.add("http://base-sepolia-faucet.skale.space");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1187947933");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.SKALE, skale);
		
		// xrpl
		EVMChainInfo xrpl = new EVMChainInfo(
				EVMChain.XRPL.toString(), 
				"XRPL EVM", 
				1440000L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("XRPL EVM Gas Token", "XRP", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.xrplevm.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.xrplevm.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1440000");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.XRPL, xrpl);
		
		// vflow
		EVMChainInfo vflow = new EVMChainInfo(
				EVMChain.VFLOW.toString(), 
				"VFlow", 
				1408L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("VFlow Gas Token", "VFY", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://vflow-rpc.zkverify.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://vflow.subscan.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1408");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.VFLOW, vflow);
		
		// symbiosis
		EVMChainInfo symbiosis = new EVMChainInfo(
				EVMChain.SYMBIOSIS.toString(), 
				"Symbiosis", 
				13863860L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Symbiosis Gas Token", "SIS", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://symbiosis.calderachain.xyz/http");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://symbiosis.calderaexplorer.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/13863860");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.SYMBIOSIS, symbiosis);
		
		// rarimo
		EVMChainInfo rarimo = new EVMChainInfo(
				EVMChain.RARIMO.toString(), 
				"Rarimo", 
				7368L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Rarimo Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://l2.rarimo.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.rarimo.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/7368");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.RARIMO, rarimo);
		
		// tranched
		EVMChainInfo tranched = new EVMChainInfo(
				EVMChain.TRANCHED.toString(), 
				"Tranched Mainnet", 
				743L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Tranched Mainnet Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://tranched-mainnet.calderachain.xyz/http");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://tranched-mainnet.calderaexplorer.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/743");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.TRANCHED, tranched);
		
		// rivalz
		EVMChainInfo rivalz = new EVMChainInfo(
				EVMChain.RIVALZ.toString(), 
				"Rivalz", 
				753L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Rivalz Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rivalz.calderachain.xyz/http");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://rivalz.calderaexplorer.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/753");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.RIVALZ, rivalz);
		
		// evoz
		EVMChainInfo evoz = new EVMChainInfo(
				EVMChain.EVOZ.toString(), 
				"Evoz Mainnet", 
				805L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Evoz Mainnet Gas Token", "EVOZ", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.evozscan.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://evozscan.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/805");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.EVOZ, evoz);
		
		// electra
		EVMChainInfo electra = new EVMChainInfo(
				EVMChain.ELECTRA.toString(), 
				"Electra Network", 
				861L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Electra Network Gas Token", "ELC", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.electranetwork.tech");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.electranetwork.tech");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/861");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ELECTRA, electra);
		
		// fushuma
		EVMChainInfo fushuma = new EVMChainInfo(
				EVMChain.FUSHUMA.toString(), 
				"Fushuma", 
				121224L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Fushuma Gas Token", "FUMA", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.fushuma.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://fumascan.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/121224");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.FUSHUMA, fushuma);
		
		// citronus
		EVMChainInfo citronus = new EVMChainInfo(
				EVMChain.CITRONUS.toString(), 
				"Citronus", 
				50000L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Citronus Gas Token", "CITRO", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.citronus.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.citronus.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/50000");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.CITRONUS, citronus);

		
		// xusd
		EVMChainInfo xusd = new EVMChainInfo(
				EVMChain.XUSD.toString(), 
				"XUSD ONE StableChain Mainnet", 
				20441L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("XUSD ONE StableChain Mainnet Gas Token", "X1", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://xusd.live");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://xusdscan.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/20441");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.XUSD, xusd);
		
		// subtensor
		EVMChainInfo subtensor = new EVMChainInfo(
				EVMChain.SUBTENSOR.toString(), 
				"Subtensor EVM", 
				964L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Subtensor EVM Gas Token", "TAO", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://lite.chain.opentensor.ai");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/964");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.SUBTENSOR, subtensor);
		
		// taker
		EVMChainInfo taker = new EVMChainInfo(
				EVMChain.TAKER.toString(), 
				"Taker Chain Mainnet", 
				1125L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Taker Chain Mainnet Gas Token", "TAKER", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc-mainnet.taker.xyz");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.taker.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1125");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.TAKER, taker);
		
		// story
		EVMChainInfo story = new EVMChainInfo(
				EVMChain.STORY.toString(), 
				"Story", 
				1514L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Story Gas Token", "IP", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://mainnet.storyrpc.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://mainnet.storyscan.xyz");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1514");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.STORY, story);
		
		// form
		EVMChainInfo form = new EVMChainInfo(
				EVMChain.FORM.toString(), 
				"Form Network", 
				478L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Form Network Gas Token", "ETH", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.form.network/http");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.form.network");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/478");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.FORM, form);
		
		// wemblockchain
		EVMChainInfo wemblockchain = new EVMChainInfo(
				EVMChain.WEMBLOCKCHAIN.toString(), 
				"WEMblockchain MainNet", 
				315L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("WEMblockchain MainNet Gas Token", "WEM", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("http://rpc.wemblockchain.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://wemscan.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/315");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.WEMBLOCKCHAIN, wemblockchain);
		
		// pointledger
		EVMChainInfo pointledger = new EVMChainInfo(
				EVMChain.POINTLEDGER.toString(), 
				"pointledger", 
				9889L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("pointledger Gas Token", "PLG", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.pointledger.net");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://exp.pointledger.net");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/9889");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.POINTLEDGER, pointledger);
		
		// action
		EVMChainInfo action = new EVMChainInfo(
				EVMChain.ACTION.toString(), 
				"Action Mainnet", 
				21000L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Action Mainnet Gas Token", "ACTN", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.actionblockchain.org");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("http://exp.actionblockchain.org");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/21000");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ACTION, action);
		
		// deepsafe
		EVMChainInfo deepsafe = new EVMChainInfo(
				EVMChain.DEEPSAFE.toString(), 
				"DeepSafe Beta Mainnet", 
				10011L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("DeepSafe Beta Mainnet Gas Token", "DEF", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://betamainnet-rpc-node-http.deepsafe.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/10011");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.DEEPSAFE, deepsafe);
		
		// puppynet
		EVMChainInfo puppynet = new EVMChainInfo(
				EVMChain.PUPPYNET.toString(), 
				"Puppynet", 
				157L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Puppynet Gas Token", "BONE", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://puppynet.shibrpc.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://puppyscan.shib.io");
				}},
				new ArrayList<String>() {{
					this.add("https://shibarium.shib.io/faucet");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/157");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.PUPPYNET, puppynet);
		
		// formicarium
		EVMChainInfo formicarium = new EVMChainInfo(
				EVMChain.FORMICARIUM.toString(), 
				"Formicarium", 
				43521L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Formicarium Gas Token", "M", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.formicarium.memecore.net");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://formicarium.memecorescan.io");
				}},
				new ArrayList<String>() {{
					this.add("https://faucet.memecore.com/formicarium");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/43521");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.FORMICARIUM, formicarium);
		
		// unite
		EVMChainInfo unite = new EVMChainInfo(
				EVMChain.UNITE.toString(), 
				"Unite", 
				88899L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Unite Gas Token", "UNITE", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://unite-mainnet.g.alchemy.com/public");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://unite-mainnet.explorer.alchemy.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/88899");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.UNITE, unite);
		
		// lux
		EVMChainInfo lux = new EVMChainInfo(
				EVMChain.LUX.toString(), 
				"Lux Mainnet", 
				96369L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Lux Mainnet Gas Token", "LUX", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://api.lux.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explore.lux-test.network");
				}},
				new ArrayList<String>() {{
					this.add("https://faucet.lux-test.network");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/96369");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.LUX, lux);
		
		// zoo
		EVMChainInfo zoo = new EVMChainInfo(
				EVMChain.ZOO.toString(), 
				"Zoo Mainnet", 
				200200L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Zoo Mainnet Gas Token", "ZOO", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://api.zoo.network");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explore.zoo-test.network");
				}},
				new ArrayList<String>() {{
					this.add("https://faucet.zoo-test.network");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/200200");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.ZOO, zoo);
		
		// owshen
		EVMChainInfo owshen = new EVMChainInfo(
				EVMChain.OWSHEN.toString(), 
				"Owshen Mainnet", 
				918273L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Owshen Mainnet Gas Token", "DIVE", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.owshen.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/918273");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.OWSHEN, owshen);

		// turbo
		EVMChainInfo turbo = new EVMChainInfo(
				EVMChain.TURBO.toString(), 
				"Turbo", 
				1313161567L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Turbo Gas Token", "TURBO", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc-0x4e45415f.aurora-cloud.dev");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://explorer.turbo.aurora.dev");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/1313161567");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.TURBO, turbo);
		
		// corn
		EVMChainInfo corn = new EVMChainInfo(
				EVMChain.CORN.toString(), 
				"Corn Maizenet", 
				21000000L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("Corn Maizenet Gas Token", "BTCN", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://maizenet-rpc.usecorn.com");
					this.add("https://rpc.ankr.com/corn_maizenet");
					this.add("https://mainnet.corn-rpc.com");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://maizenet-explorer.usecorn.com");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/21000000");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.CORN, corn);
		
		// wadzchain
		EVMChainInfo wadzchain = new EVMChainInfo(
				EVMChain.WADZCHAIN.toString(), 
				"Wadzchain Mainnet", 
				171717L, 
				BlockchainType.BORKED.toString(),
				new EVMCurrency("Wadzchain Mainnet Gas Token", "WCO", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"300000", //300k units
				new ArrayList<String>() {{
					this.add("https://rpc.wadzchain.io");
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(),
				new ArrayList<String>(),
				new ArrayList<String>() {{
					this.add("https://scan.wadzchain.io");
				}},
				new ArrayList<String>() {{
					this.add("<TODO>");
				}},
				new ArrayList<String>() {{
					this.add("https://chainlist.org/chain/171717");
				}},
				BlockchainTokenDetailsEVM.generateDummyTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.WADZCHAIN, wadzchain);
		
		// borachain
		EVMChainInfo borachain = new EVMChainInfo(
				EVMChain.BORACHAIN.toString(), 
				"Bora Chain, gaming side-chain of KLAYTN", 
				77001L, 
				BlockchainType.PUBLIC.toString(),
				new EVMCurrency("BORAChain Native Token", "BGAS", 18), 
				EVMPriceMechanism.LEGACY.toString(),30,
				"30000000000", //30 gwei
				null, // enforced min gasprice
				"21000", // pure tx
				new ArrayList<String>() {{
					this.add("https://public-node.api.boraportal.com/bora/mainnet");
					this.add("https://public-node.api.boraportal.io/bora/mainnet"); // fails to reply to node rpc version
				}}, 
				new ArrayList<String>(), // archive nodes (if available)
				new ArrayList<String>(), // flashbot nodes (if available)
				new ArrayList<String>(), // flashbot relay nodes (if available)
				new ArrayList<String>() {{
					this.add("https://scope.boraportal.com/");
				}},
				new ArrayList<String>() {{
					this.add("faucet-to-be-inserted");
				}},
				new ArrayList<String>() {{
					this.add("https://medium.com/@DAO_FE/1-add-rpc-network-name-borachain-mainnet-network-url-https-public-node-api-boraportal-io-b-59f422588926");
				}},
				BlockchainTokenDetailsEVM.generateBorachainTokenIndex(),
				BlockchainTokenDetailsEVM.generateDummyNFTIndex());
		networks.put(EVMChain.BORACHAIN, borachain);

		return networks;

	}

}