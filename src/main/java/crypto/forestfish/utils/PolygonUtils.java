...
import crypto.forestfish.objects.polygon.PolygonERC20Transaction;
import crypto.forestfish.objects.polygon.PolygonWalletBalanceReply;
import crypto.forestfish.objects.polygon.PolygonWalletTxHistoryReply;
...
public class PolygonUtils {
	...
	public static Double getMATICBalance(String walletAddress, String apiKey) {
		PolygonWalletBalanceReply reply = null;
		...
		if (null != jsonSTR) {
			reply = PolygonWalletBalanceReply.parseReply(jsonSTR);
	...
	public static int getERC20TransactionCount10k(String walletAddress, String apiKey, PolygonERC20Token token) {
		...
		while (resultCount >= 1000) {
			page++;
			PolygonWalletTxHistoryReply reply = null;
			...
			if (null != jsonSTR) {
				reply = PolygonWalletTxHistoryReply.parseReply(jsonSTR);
				LOGGER.debug("message: " + reply.getMessage());
	...
	public static Double getERC20WalletBalance10kTx(String walletAddress, String apiKey, PolygonERC20Token token) {
		...
		ArrayList<PolygonERC20Transaction> batch = new ArrayList<PolygonERC20Transaction>();
		...
		while (resultCount >= 1000) {
			...
			if (null != jsonSTR) {
				...
				LOGGER.debug("status: " + reply.getStatus());
				LOGGER.debug("result.size(): " + reply.getResult().size());
				for (PolygonERC20Transaction tx: reply.getResult()) {
					tx.update();
					batch.add(tx);
				}
		...
		for (PolygonERC20Transaction tx: batch) {
